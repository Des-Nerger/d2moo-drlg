...
union FOGASTNode
{
	enum FOGASTType nType;
	uint8_t nRawValue;
};

typedef int(__cdecl* CalcFogCallBack_t)(int, int, int, void*);
typedef int(__cdecl* CalcFogCallBack2_t)(int, void*);
...
typedef int(__cdecl* CalcGetKeyWordToNumber_t)(char* szKey);
typedef int(__cdecl* CalcGetFunctionParameterCount_t)(int nFunctionIndex);
typedef int(__cdecl* CalcGetLinkerIndex_t)(char* szText, BOOL* pOutHasResolvedToConstant, int nAST, int nKeywordNumber);
typedef int(__cdecl* CalcProcessCache_t)(int nIndex);

struct FOGExpressionParserContext
{
	enum FOGASTType tPendingOperationsASTs[64]; // 0x000
	int32_t tLinkerIndex[64];              // 0x040
	int32_t nPendingOps;                   // 0x140
	int32_t nPendingParameters;            // 0x144
};

enum : int32_t { Fog64IntStack_nCapacity = 64 };

struct Fog64IntStack
{
  int32_t tData[Fog64IntStack_nCapacity];
  int32_t nSize;
};
...
FOG_DLL_DECL int __cdecl DATATBLS_CalcEvaluateExpression(const union FOGASTNode* pExpressionBuffer, int32_t nExpressionBufferSize, CalcFogCallBack2_t fpParamCallBack, struct D2CalcCallbackInfo* pTableData, int nTableSize, void* pUserData);
...
FOG_DLL_DECL int __cdecl DATATBLS_CompileExpression(const char* szFormulaString, union FOGASTNode* pOutASTBuffer, int nOutASTBufferSize, CalcGetKeyWordToNumber_t pfnFunctionNameToId, CalcGetFunctionParameterCount_t pfnGetFunctionParameterCount, CalcGetLinkerIndex_t pfnLinkParse);
