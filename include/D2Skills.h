...
struct D2CellFile; // From D2CMP / D2Gfx
struct D2ActiveRoom;
struct D2SkillsTxt;
...
struct D2Skill
{
	...
	struct D2SkillsTxt* pSkillsTxt;				//0x00
	struct D2Skill* pNextSkill;				//0x04
...
{
	void* pMemPool;							//0x00
	struct D2Skill* pFirstSkill;				//0x04
	struct D2Skill* pLeftSkill;				//0x08
	struct D2Skill* pRightSkill;				//0x0C
	struct D2Skill* pUsedSkill;				//0x10
	uint32_t unk014;						//0x14
};

struct D2SkillTreeChart
{
	struct D2CellFile* pCellFile;				//0x00
