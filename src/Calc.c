...
// Inlined
#define IMPLEMENT_ReadFromBuffer_ON(T) \
T ReadFromBuffer_##T(const union FOGASTNode** ppBuffer) { \
	T val = *(T*)(*ppBuffer); \
	*ppBuffer += sizeof(T); \
	return val; \
}
IMPLEMENT_ReadFromBuffer_ON(int8_t);
IMPLEMENT_ReadFromBuffer_ON(int16_t);
IMPLEMENT_ReadFromBuffer_ON(int32_t);
IMPLEMENT_ReadFromBuffer_ON(uint8_t);
IMPLEMENT_ReadFromBuffer_ON(uint16_t);
IMPLEMENT_ReadFromBuffer_ON(uint32_t);

// Inlined
static void DATATBLS_EvaluateCalcCallback(struct Fog64IntStack* pCalcStack, const union FOGASTNode** pBufferCurrentPos, struct D2CalcCallbackInfo* pTableData, int nTableSize, void* pUserData)
{
	int nTableIndex = ReadFromBuffer_uint8_t(pBufferCurrentPos);
	int nResult = 0;
...
// Inlined
static void DATATBLS_EvaluateUsingConstantFromBuffer(struct Fog64IntStack* pCalcStack, enum FOGASTType nASTType, const union FOGASTNode** pBufferCurrentPos, CalcFogCallBack2_t fpParamCallBack, void* pUserData)
{
	...
	case AST_Callback_Param_UInt8:
		nCallbackResult = fpParamCallBack(ReadFromBuffer_uint8_t(pBufferCurrentPos), pUserData);
		break;
	case AST_Callback_Param_UInt16:
		nCallbackResult = fpParamCallBack(ReadFromBuffer_uint16_t(pBufferCurrentPos), pUserData);
		break;
	case AST_Callback_Param_UInt32:
		nCallbackResult = fpParamCallBack(ReadFromBuffer_uint32_t(pBufferCurrentPos), pUserData);
		break;
	case AST_Raw_Int8:
		nCallbackResult = ReadFromBuffer_int8_t(pBufferCurrentPos); // NOLINT
		break;
	case AST_Raw_Int16:
		nCallbackResult = ReadFromBuffer_int16_t(pBufferCurrentPos);
		break;
	case AST_Raw_Int32:
		nCallbackResult = ReadFromBuffer_int32_t(pBufferCurrentPos);
		break;
	default:
		D2_UNREACHABLE;
	}
...
// Inlined
static void DATATBLS_EvaluateBinaryOperator(struct Fog64IntStack* pCalcStack, enum FOGASTType nAST)
{
...
int __cdecl DATATBLS_CalcEvaluateExpression(const union FOGASTNode* pExpressionBuffer, int32_t nExpressionBufferSize, CalcFogCallBack2_t fpParamCallBack, struct D2CalcCallbackInfo* pTableData, int nTableSize, void* pUserData)
{
	...
	const union FOGASTNode* pBufferCurrentPos = pExpressionBuffer;
	...
	#define ReachedEndOfBuffer() (pBufferCurrentPos - pExpressionBuffer >= nExpressionBufferSize)

	struct Fog64IntStack tCalcStack;
	tCalcStack.nSize = 0;

	while (!ReachedEndOfBuffer())
	{
		uint8_t nAST = ReadFromBuffer_uint8_t(&pBufferCurrentPos);
		switch (nAST)
		{
		...
		case AST_Raw_Int32:
		{
			...
			DATATBLS_EvaluateUsingConstantFromBuffer(&tCalcStack, (enum FOGASTType)nAST, &pBufferCurrentPos, fpParamCallBack, pUserData);
			break;
		}
		case AST_LessThan:
		case AST_GreaterThan:
		...
		case AST_Division:
		case AST_Power:
			DATATBLS_EvaluateBinaryOperator(&tCalcStack, (enum FOGASTType)nAST);
			break;
		case AST_Negate:
...
static union FOGASTNode* __cdecl DATATBLS_ExpressionBuffer_PushConstant(union FOGASTNode* pExpressionBufferPos, union FOGASTNode* pExpressionBufferStart, int szBufferSize, struct FOGExpressionParserContext* pCalc, int32_t nValue, bool bCallback)
{
	...
	if (INT8_MIN <= nValue && nValue <= INT8_MAX)
	{
	...
	else if (INT16_MIN <= nValue && nValue <= INT16_MAX)
	{
...
// Helper function
static int GetASTNodeParameterCount(enum FOGASTType nASTType, int nTokenAssociatedValue, CalcGetFunctionParameterCount_t pfnGetFunctionParameterCount)
{
	switch (nASTType)
	{
	case AST_CallbackTable:
...
//1.10f: 0x6FF712F0
static const enum FOGASTType aASTOperatorPrecedence[24] = {
...
union FOGASTNode* __cdecl DATATBLS_Evaluate_HandleNewOp(union FOGASTNode* pASTBufferPos, union FOGASTNode* pASTBufferStart, int nASTBufferSize, struct FOGExpressionParserContext* pContext, enum FOGASTType nNewASTType, CalcGetFunctionParameterCount_t pfnGetFunctionParametersCount, int nTokenAssociatedValue)
{
	...
	{
		const enum FOGASTType nPreviousASTType = pContext->tPendingOperationsASTs[pContext->nPendingOps - 1];
...
// Helper function
static void DATATBLS_ResolveConstantLink(char* szLinkName, struct FOGExpressionParserContext* pCalc, enum FOGCalcExpressionParserTokenType* pOutTokenType, int* pTokenAssociatedValue, CalcGetLinkerIndex_t pfnLinkParse)
{
...
// Helper function
static const char* DATATBLS_ParseLinkToken(const char* szExpression, struct FOGExpressionParserContext* pCalc, enum FOGCalcExpressionParserTokenType* pOutTokenType, int* pTokenAssociatedValue, CalcGetLinkerIndex_t pfnLinkParse)
{
...
// Helper function
static const char* DATATBLS_ParseSublinkToken(const char* szExpression, struct FOGExpressionParserContext* pCalc, enum FOGCalcExpressionParserTokenType* pOutTokenType, int* pTokenAssociatedValue, CalcGetLinkerIndex_t pfnLinkParse)
{
	D2_ASSERT(*szExpression == '.');
...
// Helper function
static const char* DATATABLS_ParseSubExpression(const char* szExpression, struct FOGExpressionParserContext* pCalc, enum FOGCalcExpressionParserTokenType* pOutTokenType, int* pTokenAssociatedValue, CalcGetKeyWordToNumber_t pfnFunctionNameToId, CalcGetLinkerIndex_t pfnLinkParse)
{
	// Read token
	...
	if (*szExpression == '(')
	{
		const int nFunctionId = pfnFunctionNameToId(tSubExpressionBuffer);
		if (nFunctionId)
		{
...
const char* DATATABLS_ParseExpressionToken(const char* szExpression, enum FOGCalcExpressionParserTokenType* pOutTokenType, struct FOGExpressionParserContext* pCalc, int* pTokenAssociatedValue, CalcGetKeyWordToNumber_t pfnFunctionNameToId, CalcGetLinkerIndex_t pfnLinkParse)
{
	*pTokenAssociatedValue = 0;
...
int __cdecl DATATBLS_CompileExpression(const char* szFormulaString, union FOGASTNode* pOutASTBuffer, int nOutASTBufferSize, CalcGetKeyWordToNumber_t pfnFunctionNameToId, CalcGetFunctionParameterCount_t pfnGetFunctionParameterCount, CalcGetLinkerIndex_t pfnLinkParse)
{
	...
	union FOGASTNode* pASTBufferPos = pOutASTBuffer;
	enum FOGCalcExpressionParserTokenType nToken = TOKEN_PAREN_OPEN;
	int32_t nTokenAssociatedValue = 0;
	struct FOGExpressionParserContext tContext;
		...
		)
	{
		...
		case TOKEN_PAREN_CLOSE:
			...
			while (1)
			{
				const enum FOGASTType nPreviousASTType = tContext.tPendingOperationsASTs[tContext.nPendingOps - 1];
