...
//	case PROP_AFFIXES:
//	{
//		struct D2MagicAffixTxt* pRecord = (struct D2MagicAffixTxt*)pMods;
//		if (pRecord == NULL)
//			return;
//
//		struct D2Property* pProperties = pRecord->pProperties;
...
//	case PROP_RUNES:
//	{
//		struct D2GemsTxt* pRecord = (FileGemsTable*)pMods;
//		if (pRecord == NULL)
//			return;
//
//		struct D2Property* pProperties = pRecord->pProperties[nPropSet];
...
//	case PROP_GRADES:
//	{
//		struct D2QualityItemsTxt* pRecord = (FileQualityitemsTable*)pMods;
//		if (pRecord == NULL)
//			return;
//
//		struct D2Property* pProperties = pRecord->pProperties;
...
//		{
//			struct D2UniqueItemsTxt* pRecord = INLINE_GetUniqueItemRecord(nSpecialIndex);
//			if (pRecord == NULL)
//				break;
//
//			struct D2Property* pProperties = pRecord->pProperties;
...
//		if (VALID_UNIQUEITEM(nSpecialIndex))
//		{
//			struct D2SetItemsTxt* pRecord = INLINE_GetSetItemRecord(nSpecialIndex);
//			if (pRecord == NULL)
//				return;
//
//			struct D2Property* pProperties = pRecord->pProperties;
...
//			}
//
//			static struct D2PropertySetFunc pSetItemFuncs[7] =
//			{
//				{ 0,					STATLIST_MAGIC },
//				{ 0,					STATLIST_MAGIC },
...
//			{
//				struct D2PropertySetFunc* pSet = &pSetItemFuncs[pRecord->nAddFunc ? (j / 2) + 2 : 0];
...
struct D2CalcCallbackInfo off_6FDE3BA0[] =
{
...
static const struct D2PropertyAssign stru_6FDE3160[] =
{
...
void __stdcall ITEMMODS_AssignProperty(int nType, struct D2Unit* a2, struct D2Unit* pItem, const void* pMods, int nPropSet, int nApplyType)
{
	...
	struct D2Property* pProperties[3] = {};
	struct D2Property* pProperty = NULL;
	int nProperties = 0;
	int nFileIndex = 0;
	int nState = 0;
	int nFlag = 0;
...
#define MAX_MAGICITEM_PROPERTIES 3
			...
			pProperty = &((struct D2MagicAffixTxt*)pMods)->pProperties[nMod];
			break;
		}
...
#define MAX_UNIQUE_ITEM_PROPERTIES 12
		...
		case PROPMODE_RUNE:
		{
			pProperties[0] = ((struct D2GemsTxt*)pMods)->pProperties[0];
			pProperties[1] = ((struct D2GemsTxt*)pMods)->pProperties[1];
			pProperties[2] = ((struct D2GemsTxt*)pMods)->pProperties[2];
...
#define MAX_GEMITEM_PROPERTIES 3
		...
		{
			pProperty = (struct D2Property*)pMods;
			break;
		}
...
BOOL __stdcall ITEMMODS_UpdateRuneword(struct D2Unit* pUnit, struct D2Unit* pItem, int nUnused)
{
	...
	const struct D2RunesTxt* pRunesTxtRecord = ITEMS_GetRunesTxtRecordFromItem(pUnit);
...
int __fastcall sub_6FD98220(int nMin, int nMax, int nUnused, void* pUserData)
{
	...
	if (pCalc)
	{
		v5 = (struct D2Seed*)(pCalc->field_0 + 32);
		if (nMin < nMax)
		{
