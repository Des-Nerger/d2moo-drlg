...
void __fastcall SEED_InitSeed(struct D2Seed* pSeed)
{
	pSeed->nLowSeed = 1;
	pSeed->nHighSeed = 666;
}
...
void __fastcall SEED_InitLowSeed(struct D2Seed* pSeed, int nLowSeed)
{
...
uint32_t  __fastcall SEED_GetLowSeed(struct D2Seed* pSeed)
{
	return pSeed->nLowSeed;
}

//D2Common.0x6FDAEAE0 (#10921)
void __fastcall SEED_SetSeeds(struct D2Seed* pSeed, uint32_t  nLowSeed, uint32_t  nHighSeed)
{
	pSeed->nLowSeed = nLowSeed;
...
void __fastcall SEED_GetSeeds(struct D2Seed* pSeed, uint32_t* pLowSeed, uint32_t* pHighSeed)
{
...
uint32_t __fastcall SEED_GetHighSeed(struct D2Seed* pSeed)
{
