...
static const struct D2UnkPath stru_6FDD2158[25] =
{
	{ 5, 4, 6 },
	{ 4, 5, 6 },
	{ 4, 3, 5 },
...
static const struct D2UnkPath2 byte_6FDD2288[25] =
{
	{ 4, 6, -1 },
	{ 4, 6, -1 },
	{ 4, 2, 6 },
...
const struct D2Coord gatDirectionToOffset[8] = {
	{ 1, 0},
	{ 1, 1},
	{ 0, 1},
	{ -1, 1},
...
//D2Common.0x6FDAA720
int __cdecl sub_6FDAA720(struct D2PathInfo* pPathInfo)
{
	struct D2DynamicPath* pDynamicPath = pPathInfo->pDynamicPath;
	...
	if (pPathInfo->nDistMax > 0)
	{
		...
		struct D2PathPoint tLastSegmentEndCoord = pPathInfo->tStartCoord;
		bool bSegmentEndAlreadyAdded = false;
		
		struct D2PathPoint tCurCoords = tLastSegmentEndCoord;
		int nCurDistance;
		for(nCurDistance = 0;nCurDistance < pPathInfo->nDistMax; nCurDistance++)
		{
			// We reached the destination, stop here
			if (0 == memcmp(&tCurCoords, &tTargetCoord, sizeof(struct D2PathPoint)))
				break;
...
BOOL __cdecl sub_6FDAA880(struct D2PathInfo* pPathInfo, int* pTestDir, struct D2PathPoint pPoint, int* pDirection)
{
	struct D2Unit* pUnit = pPathInfo->pDynamicPath->pUnit;
...
BOOL __cdecl PATH_CheckTestDirForCollisions(const struct D2PathInfo* pPathInfo, const int* pTestDir, struct D2PathPoint* pPoints, int* pFreeOfCollisionDirIndex)
{
	struct D2Unit* pUnit = pPathInfo->pDynamicPath->pUnit;
...
BOOL PATH_PreparePathTargetForPathUpdate(struct D2PathInfo* pPathInfo)
{
	...
	struct D2PathPoint aTestDirPathPoints[3] = { pPathInfo->tTargetCoord, pPathInfo->tTargetCoord, pPathInfo->tTargetCoord };

	sub_6FDAB7D0(pTestDir, 0 /*unused*/, pPathInfo->tTargetCoord, pPathInfo->tStartCoord);
	while (1)
	{
		if (0 == memcmp(&pPathInfo->tStartCoord, &aTestDirPathPoints[0], sizeof(struct D2PathPoint)))
		{
			return 0;
		}
...
int __cdecl PATH_Toward_6FDAA9F0(struct D2PathInfo *pPathInfo)
{
	struct D2DynamicPath* pPath = pPathInfo->pDynamicPath;
	...
	struct D2PathPoint nNextPoint = pPathInfo->tTargetCoord;
	...
	{
		struct D2PathPoint tPoint = pPathInfo->tStartCoord;
		struct D2PathPoint tCurCoords = pPathInfo->tStartCoord;
		if (0 != memcmp(&pPathInfo->tStartCoord, &nNextPoint, sizeof(struct D2PathPoint)))
		{
		...
		if (pPathInfo->nDistMax > 0)
		{
			struct D2PathPoint* pPathPointsEnd = &pPath->PathPoints[pPath->dwPathPoints];
			do
			{
				if (0 == memcmp(&tCurCoords, &nNextPoint, sizeof(struct D2PathPoint)))
					break;
				...
				{
					if (0 != memcmp(&tPoint, &pPathInfo->tStartCoord, sizeof(struct D2PathPoint)))
					{
...
BOOL __cdecl PATH_CheckCollisionsToNextPosition(struct D2DynamicPath* pDynamicPath, struct D2PathPoint* pGameCoord)
{
	struct D2PathPoint pPoint = {};
...
BOOL __cdecl PATH_RayTrace(struct D2DynamicPath* pDynamicPath, struct D2PathPoint* pPathDestination, struct D2PathPoint tStartCoord)
{
	...
	struct D2ActiveRoom* pPathRoom = pDynamicPath->pRoom;
	struct D2PathPoint tTargetPoint = *pPathDestination;

	struct D2PathPoint tLastPointWithoutCollision = tStartCoord;
	struct D2PathPoint tCurPoint = tStartCoord;
	const int nDeltaX = tTargetPoint.X - tStartCoord.X;
	const int nDeltaY = tTargetPoint.Y - tStartCoord.Y;
	const int nPointsX = abs(nDeltaX) + 1;
	const int nPointsY = abs(nDeltaY) + 1;
...
//D2Common.0x6FDAB0B0
int __cdecl sub_6FDAB0B0(struct D2PathInfo* pPathInfo)
{
...
//D2Common.0x6FDAB0C0
int __cdecl PATH_BackupTurn_Compute(struct D2PathInfo* pPathInfo)
{
	int result = 0;
	int v5 = 0;
...
int __cdecl PATH_Straight_Compute(struct D2PathInfo* pPathInfo)
{
	pPathInfo->pDynamicPath->dwCurrentPointIdx = 0;
	...
	if (nPathPoints)
	{
		struct D2PathPoint tPathLastPoint = pPathInfo->pDynamicPath->PathPoints[nPathPoints - 1];
		if (sub_6FDABA50(tPathLastPoint, pPathInfo->tTargetCoord) <= pPathInfo->field_14 // If heuristic distance is too small
			&& 0 != memcmp(&tPathLastPoint, &pPathInfo->tStartCoord, sizeof(struct D2PathPoint)))								 // and we couldn't reach the target
		{
	...
	const int nDiffX = abs(pPathInfo->tStartCoord.X - pPathInfo->tTargetCoord.X);
	const int nDiffY = abs(pPathInfo->tStartCoord.Y - pPathInfo->tTargetCoord.Y);
	const int nSquaredDistance = nDiffX * nDiffX + nDiffY * nDiffY;

	if (nSquaredDistance <= nMaxDistSquared)
	{
		int nAStarPathPoints = PATH_AStar_ComputePath(pPathInfo);
		if (nAStarPathPoints)
		{
...
int __cdecl PATH_Leap_6FDAB1E0(struct D2PathInfo* pPathInfo)
{
	struct D2DynamicPath* pDynamicPath = pPathInfo->pDynamicPath;
	pDynamicPath->dwCurrentPointIdx = 0;
	struct D2PathPoint pGameCoord = pPathInfo->tTargetCoord;
	pDynamicPath->PathPoints[0] = pGameCoord;
	PATH_CheckCollisionsToNextPosition(pDynamicPath, &pGameCoord);
	if (0 == memcmp(&pPathInfo->tStartCoord, &pGameCoord, sizeof(struct D2PathPoint)))
		return 0;
...
//D2Common.0x6FDAB240
int __cdecl PATH_Knockback_Client(struct D2PathInfo* pPathInfo)
{
	pPathInfo->pDynamicPath->dwCurrentPointIdx = 0;
...
//D2Common.0x6FDAB270
int __cdecl PATH_Knockback_Server(struct D2PathInfo* pPathInfo)
{
	struct D2DynamicPath* pDynamicPath = pPathInfo->pDynamicPath;
	int nHalfDist = (pDynamicPath->nDist >> 1) + 1;
	struct D2PathPoint tFinalCoords;
	struct D2Unit* pTargetUnit = pDynamicPath->pTargetUnit;
	if (pTargetUnit)
	{
		...
		const int nMaxAbsDelta = MAX(abs(nDeltaX), abs(nDeltaY));
		if (nMaxAbsDelta)
		{
	...
	if (0 == memcmp(&tFinalCoords, &pPathInfo->tStartCoord, sizeof(struct D2PathPoint)))
	{
		return 0;
	}
	pDynamicPath->PathPoints[0] = tFinalCoords;
...
//D2Common.0x6FDAB3C0
signed int __cdecl PATH_ComputePathBlessedHammer_6FDAB3C0(struct D2DynamicPath* pDynamicPath)
{
	...
	struct D2PathPoint previousPoint = { PATH_FromFP16(dwOriginPrecisionX) , PATH_FromFP16(dwOriginPrecisionY) };
	...
	const int nbPointsToGenerate = D2DynamicPath_MAXPATHLEN - 1;
	for (int curPointIdx = 0; curPointIdx < nbPointsToGenerate; curPointIdx++)
	{
		...
		if (0 != memcmp(&newPoint, &previousPoint, sizeof(struct D2PathPoint)))
		{
...
//D2Common.0x6FDAB4A0
int __cdecl PATH_ComputePathChargedBolt_6FDAB4A0(struct D2DynamicPath* pDynamicPath, struct D2Seed* pSeed)
{
	struct D2PathPoint nPrevPoint;
...
//D2Common.0x6FDAB610
int __cdecl sub_6FDAB610(int nX1, int nY1, int nX2, int nY2)
{
	int nAbsDiffX = 0;
	int nAbsDiffY = 0;
	int nDiffX = 0;
...
int __cdecl sub_6FDAB6A0(struct D2PathPoint pPoint1, struct D2PathPoint pPoint2)
{
	return sub_6FDAB610(pPoint1.X, pPoint1.Y, pPoint2.X, pPoint2.Y);
}

//D2Common.0x6FDAB750
int __cdecl sub_6FDAB750(int nX1, int nY1, int nX2, int nY2)
{
...
int __cdecl sub_6FDAB770(struct D2PathPoint pPoint1, struct D2PathPoint pPoint2)
{
	return stru_6FDD2158[sub_6FDAB6A0(pPoint1, pPoint2)].unk0x00;
}

//D2Common.0x6FDAB790
void __cdecl PATH_GetDirections_6FDAB790(int* pTestDir, struct D2PathPoint pPoint1, struct D2PathPoint pPoint2)
{
...
void __cdecl sub_6FDAB7D0(int* pTestDir, int nUnused, struct D2PathPoint pPoint1, struct D2PathPoint pPoint2)
{
...
//D2Common.0x6FDABA50
int __cdecl sub_6FDABA50(struct D2PathPoint pPoint1, struct D2PathPoint pPoint2)
{
...
//D2Common.0x6FDAC170
int __cdecl PATH_SimplifyToLines_6FDAC170(struct D2PathPoint* pOutPathPoints, struct D2PathPoint* pInputPoints, struct D2PathPoint tStartCoord, signed int nbTempPoints)
{
	if (nbTempPoints >= 2)
	{
		int nbOutPoints = 0;
...
int __cdecl PATH_ComputeDirection(int nX1, int nY1, int nX2, int nY2)
{
...
//D2Common.0x6FDAC760
int __cdecl PATH_ComputeDirectionFromPreciseCoords_6FDAC760(DWORD dwStartPrecisionX, DWORD dwStartPrecisionY, DWORD dwTargetPrecisionX, DWORD dwTargetPrecisionY)
{
	struct D2Coord pCoords = {};
	int nDirection = 0;
...
void __cdecl PATH_ComputeVelocityAndDirectionVectorsToNextPoint(struct D2DynamicPath* pPath, BOOL bNormalizeDirectionIfSamePos, BOOL bForceDirectionNormalization)
{
	...
	struct D2Coord tDirectionVector;
	int nDirection;
...
//TODO: Find a name
int __cdecl D2Common_10236(struct D2Unit* pUnit, int a2)
{
	struct D2DynamicPath* pDynamicPath = NULL;
	int nResult = 0;
...
void sub_6FD5CEB0(struct D2DynamicPath* pDynamicPath, int nBaseVelocity)
{
	if (nBaseVelocity <= 0)
	{
		nBaseVelocity = 1024;
	...
	int nAcceleration = pDynamicPath->dwAcceleration;
	if (nAcceleration)
	{
...
BOOL sub_6FD5DB70(struct D2DynamicPath* pDynamicPath)
{
	...
	if (!pTargetUnit)
	{
		const struct D2PathPoint tGameCoords = D2FP32_16_ToPathPoint(pDynamicPath->tGameCoords);
		if (pDynamicPath->dwCurrentPointIdx >= pDynamicPath->dwPathPoints
			&& 0 != memcmp(&tGameCoords, &pDynamicPath->tFinalTargetCoord, sizeof(struct D2PathPoint)))
		{
	...
	if (pTargetUnit)
	{
		...
		{
			const int nAbsDiffX = abs(pDynamicPath->tPrevTargetCoord.X - a3.X);
			const int nAbsDiffY = abs(pDynamicPath->tPrevTargetCoord.Y - a3.Y);
			if (nAbsDiffX <= 5 && nAbsDiffY <= 5)
			{
				const struct D2PathPoint tGameCoords = D2FP32_16_ToPathPoint(pDynamicPath->tGameCoords);
				if (pDynamicPath->dwCurrentPointIdx < pDynamicPath->dwPathPoints
					|| 0 == memcmp(&pDynamicPath->tFinalTargetCoord, &tGameCoords, sizeof(struct D2PathPoint)) )
				{
					return TRUE;
				}
			}
...
BOOL __cdecl D2Common_10226(struct D2Unit* pUnit, signed int a2)
{
	...
	// TODO name
		auto IsUnitSomething = [](struct D2Unit* pUnit, struct D2DynamicPath* pDynamicPath) -> bool {
...
#endif
		{
				...
				&& pDynamicPath->dwCurrentPointIdx < pDynamicPath->dwPathPoints)
			{
				...
				{
					struct D2Unit* nUnused = nullptr;
					D2_MAYBE_UNUSED(nUnused);
					union D2FP32_16 tCoord;
...
//TODO: Find a name
BOOL __cdecl D2Common_10227(struct D2Unit* pUnit)
{
	struct D2DynamicPath*  pDynamicPath = pUnit->pDynamicPath;
...
BOOL __cdecl D2Common_10229(struct D2DynamicPath *pDynamicPath, struct D2Unit *pUnit, struct D2ActiveRoom *pDestRoom, uint32_t nDestX, uint32_t nDestY)
{
	struct D2PathPoint tCoords = { (uint16_t)(nDestX), (uint16_t)(nDestY) };
...
void __cdecl PATH_RecacheRoom(struct D2DynamicPath* pDynamicPath, struct D2ActiveRoom* pHintRoom)
{
	...
	struct D2ActiveRoom* pRoomAtLocation = DUNGEON_GetRoomAtPosition(pDynamicPath->pRoom, pDynamicPath->tGameCoords.wPosX, pDynamicPath->tGameCoords.wPosY);
	if (!pRoomAtLocation)
	{
