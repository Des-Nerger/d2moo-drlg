...
struct D2PathPoint sgctZeroGameCoord = { 0,0 };

bool COORD_TEST_EQUAL(struct D2PathPoint lhs, struct D2PathPoint rhs) { return 0 == memcmp(&lhs, &rhs, sizeof(struct D2PathPoint)); }


struct TanToDirectionLutEntry {
	struct D2Coord dirVector;
	uint32_t nAngle;
};

//1.00:  D2Common.0x1008AF38
//1.10f: D2Common.0x6FDD2598 
struct TanToDirectionLutEntry lutTanToDirection[] = {
	{{ 0x0000, 0x1000 } , 0 },
	{{ 0x0020, 0x0FFF } , 0 },
	{{ 0x0040, 0x0FFF } , 0 },
	{{ 0x0060, 0x0FFE } , 0 },
...
void __fastcall PATH_GetDirectionVector(struct D2Coord* pDirectionVector, int* pOutDirection, DWORD dwStartPrecisionX, DWORD dwStartPrecisionY, DWORD dwTargetPrecisionX, DWORD dwTargetPrecisionY)
{
	...
	{
		SWAP(dwMinPrecisionX, dwMinPrecisionY);
		SWAP(dwMaxPrecisionX, dwMaxPrecisionY);
	...
	const struct TanToDirectionLutEntry* pLutEntry = &lutTanToDirection[tangent];

	uint8_t nAngle = (uint8_t)pLutEntry->nAngle;
	if (!bDeltaXGreaterThanY)
	{
		pDirectionVector->nX = pLutEntry->dirVector.nX;
		pDirectionVector->nY = pLutEntry->dirVector.nY;
	}
	else
	{
		pDirectionVector->nY = pLutEntry->dirVector.nX;
		pDirectionVector->nX = pLutEntry->dirVector.nY;
		nAngle = (-1 - nAngle) & 0xF;
	}
...
BOOL __fastcall sub_6FDACEC0(struct D2DynamicPath* pDynamicPath, union D2FP32_16* a2, struct D2Unit** pUnit)
{
	...
	uint32_t v25; // ebx
	uint32_t v26; // ecx
	struct D2PathPoint v27; // edx
	int v28; // ebx
	uint16_t nCollidedWithMask; // ax
	signed int v35; // eax
	uint32_t v36; // edx
	int v37; // [esp+10h] [ebp-48h]
	struct D2PathPoint v38; // [esp+10h] [ebp-48h]
	int v40; // [esp+18h] [ebp-40h]
	int v41; // [esp+18h] [ebp-40h]
	int v42; // [esp+1Ch] [ebp-3Ch]
	int v43; // [esp+20h] [ebp-38h]
	struct D2Unit* v44; // [esp+24h] [ebp-34h]
	struct D2PathPoint v45; // [esp+28h] [ebp-30h]
	struct D2PathPoint v46; // [esp+30h] [ebp-28h]
	...
	*pUnit = 0;
	...
	if (HIWORD(v16) != HIWORD(v18) || (WORD)v19 != HIWORD(v55))
	{
		...
		while (1)
		{
			if (0 == memcmp(&(struct D2PathPoint){ v27.X, v45.Y }, &v38, sizeof(struct D2PathPoint)))
			{
			LABEL_58:
				...
				a2->dwPrecisionX = v24;
				a2->dwPrecisionY = v25;
				*pUnit = (struct D2Unit*)pDynamicPath->nSavedStepsCount;
				goto LABEL_61;
			}
			v28 = v48 + v23;
			v53 = v22 + v49;
			...
			v46.X = (v48 + v23) >> 16;
			v46.Y = (v22 + v49) >> 16;
			if (0 != memcmp(&(struct D2PathPoint){ v27.X,v45.Y }, &v46, sizeof(struct D2PathPoint)))
				break;
		LABEL_56:
			v22 = v53;
			v23 = v28;
		...
		LABEL_53:
			...
			if (pDynamicPath->nSavedStepsCount >= D2DynamicPath_PATH_MAX_STEP_LEN)
			{
				v24 = v50;
				v25 = v51;
				goto LABEL_58;
...
BOOL __fastcall sub_6FDAD5E0(struct D2DynamicPath* pDynamicPath, struct D2ActiveRoom* pDestRoom, struct D2PathPoint tDest)
{
	...
	{
		if (0 == memcmp(&tDest, &(struct D2PathPoint){ 0, 0 }, sizeof(struct D2PathPoint)))
		{
		...
		{
			struct D2PathPoint tPathPos = { pDynamicPath->tGameCoords.wPosX , pDynamicPath->tGameCoords.wPosY };
...
void PATH_StopMovement(struct D2DynamicPath* pDynamicPath)
{
...
void __stdcall D2Common_10235_PATH_UpdateRiderPath(struct D2Unit* pRiderUnit, struct D2Unit* pMountUnit)
{
	...
	struct D2DynamicPath* pRiderPath = pRiderUnit->pDynamicPath;
	struct D2DynamicPath* pMountPath = pMountUnit->pDynamicPath;
	...
	struct D2ActiveRoom* pMountRoom = pMountPath->pRoom;
	...
	if (pRiderPath->pUnit && (pRiderPath->dwFlags & PATH_UNKNOWN_FLAG_0x00001))
	{
		...
		struct D2ActiveRoom* pRidersUpToDateRoom = COLLISION_GetRoomBySubTileCoordinates(pRiderPath->pRoom, pRiderPath->tGameCoords.wPosX, pRiderPath->tGameCoords.wPosY);
