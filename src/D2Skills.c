...
struct D2CalcCallbackInfo off_6FDE5804[] =
{
...
struct D2Skill* __fastcall SKILLS_GetHighestLevelSkillFromSkillId(struct D2Unit* pUnit, int nSkillId)
{
	...
	if (pUnit->pSkills)
	{
		for (struct D2Skill* pSkill = pUnit->pSkills->pFirstSkill; pSkill; pSkill = pSkill->pNextSkill)
		{
...
void __fastcall D2COMMON_SKILLS_RemoveSkill_6FDAFFF0(struct D2Unit* pUnit, int nSkillId, BOOL bDecrementAndCheckSkillLevel, const char* szFile, int nLine)
{
	...
	const struct D2Skill* pLeftSkill = pUnit->pSkills->pLeftSkill;
	if (pLeftSkill)
	{
	...
	const struct D2Skill* pRightSkill = pUnit->pSkills->pRightSkill;
	if (pRightSkill)
	{
	...
	const struct D2Skill* pUsedSkill = pUnit->pSkills->pUsedSkill;
	if (pUsedSkill)
	{
	...
	struct D2Skill* pPreviousSkill = nullptr;
	struct D2Skill* pSkill = pUnit->pSkills->pFirstSkill;
...
struct D2Skill* __stdcall SKILLS_AddSkill(struct D2Unit* pUnit, int nSkillId)
{
	...
	if (pUnit)
	{
		...
		if (pSkillsTxtRecord)
		{
			...
			if (pUnit->pSkills)
			{
				pSkill = D2_CALLOC_STRC_POOL(pUnit->pSkills->pMemPool, struct D2Skill);
				...
				if (pPreviousSkill)
				{
					for (struct D2Skill* i = pPreviousSkill->pNextSkill; i; i = i->pNextSkill)
					{
						pPreviousSkill = i;
					}
		...
		if (pSkillsTxtRecord)
		{
			...
			if (pSkillList)
			{
				...
				else
				{
					...
					{
						pSkill = D2_CALLOC_STRC_POOL(pSkillList->pMemPool, struct D2Skill);
						pSkill->pSkillsTxt = pSkillsTxtRecord;
...
void __stdcall SKILLS_SetLeftActiveSkill(struct D2Unit* pUnit, int nSkillId, D2UnitGUID nOwnerGUID)
{
	...
	struct D2Skill* pSkill = UNITS_GetStartSkill(pUnit);
	if (pSkill)
	{
...
BOOL __fastcall D2Common_SKILLS_CheckShapeRestriction_6FDB1380(struct D2Unit* pUnit, struct D2Skill* pSkill)
{
	...
	struct D2SkillsTxt* pSkillsTxt = pSkill->pSkillsTxt;
...
int __stdcall D2Common_11014_ConvertShapeShiftedMode(int nArrayIndex, int nMonsterId)
{
	...
	while (1)
	{
		struct D2MonStats2Txt* pMonStats2TxtRecord = UNITS_GetMonStats2TxtRecordFromMonsterId(nMonsterId);
