...
//D2Common.0x6FDE21B8
const struct D2EnvironmentCycle gNormalEnvironmentCycle[NUM_ENVIRONMENT_CYCLES] =
{
...
//D2Common.0x6FDE2200
const struct D2EnvironmentCycle gAct4EnvironmentCycle[NUM_ENVIRONMENT_CYCLES] =
{
...
const struct D2EnvironmentCycle gEclipseEnvironmentCycle[NUM_ENVIRONMENT_CYCLES] =
{
...
static const struct D2EnvironmentCycle* ENVIRONMENT_GetCycle(int nCycle, int nAct, bool bEclipse)
{
...
struct D2DrlgEnvironment* __cdecl ENVIRONMENT_AllocDrlgEnvironment(void* pMemPool)
{
	...
	const struct D2EnvironmentCycle* pEnvironmentCycle = &gNormalEnvironmentCycle[pEnvironment->nCycleIndex];
...
{
	return (uint8_t)(D2Lerp(nThis,nNext,lerpRatio) + 0.5); // Round half up // NOLINT(bugprone-incorrect-roundings)
}
...
void __cdecl ENVIRONMENT_UpdateLightColor(struct D2DrlgEnvironment* pEnvironment, int nAct)
{
	...
	const struct D2EnvironmentCycle* pCurrEnvironmentCycle = ENVIRONMENT_GetCycle(nCycleIndex, nAct, pEnvironment->bEclipse);
	const struct D2EnvironmentCycle* pNextEnvironmentCycle = ENVIRONMENT_GetCycle(nNextCycleIndex, nAct, pEnvironment->bEclipse);
...
void __cdecl ENVIRONMENT_UpdateTicks(struct D2DrlgEnvironment* pEnvironment, int nActNo)
{
	...
	const struct D2EnvironmentCycle* pNextEnvCycle = ENVIRONMENT_GetCycle(nNextEnvCycle, nActNo, pEnvironment->bEclipse);
...
BOOL __cdecl ENVIRONMENT_UpdateCycleIndex(struct D2DrlgAct* pAct, int nActNo)
{
	...
	if (pEnvironment)
	{
		...
		if (pEnvironment->nTimeRate)
		{
			...
			const int nDiff = abs(nNext - pEnvironment->nPrev);
...
void __cdecl ENVIRONMENT_NextEnvCycle(struct D2DrlgAct* pAct, struct D2ActiveRoom* pRoom)
{
	...
	const struct D2EnvironmentCycle* pEnvironmentCycle = NULL;
	if (pEnvironment->bEclipse)
	{
