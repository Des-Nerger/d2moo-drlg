...
void __fastcall DATATBLS_CubeMainInputLinker(char* pSrc, void* pRecord, int nOffset, int nPosition, int nTxtRow, int nTxtColumn)
{
	...
	{
		DATATBLS_CubeMainInputParser(&((struct D2CubeMainTxt*)pRecord)->pInputItem[nOffset], pSrc, nTxtRow, nOffset);
	}
}
...
void __fastcall DATATBLS_CubeMainOutputLinker(char* pSrc, void* pRecord, int nOffset, int nPosition, int nTxtRow, int nTxtColumn)
{
	if (pRecord && pSrc && *pSrc)
	{
		DATATBLS_CubeMainOutputParser(&((struct D2CubeMainTxt*)pRecord)->pOutputItem[nOffset], pSrc, nTxtRow, nOffset);
	}
}
...
void __fastcall DATATBLS_LoadCubeMainTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pCubeMainTxt = (struct D2CubeMainTxt*)DATATBLS_CompileTxt(hArchive, "cubemain", pTbl, &sgptDataTables->nCubeMainTxtRecordCount, sizeof(struct D2CubeMainTxt));
}
