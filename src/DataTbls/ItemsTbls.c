...
void __fastcall DATATBLS_LoadItemsTxt(HD2ARCHIVE hArchive)
{
	...
	struct D2ItemsTxt* pMisc = NULL;
	struct D2ItemsTxt* pItems = NULL;
	int nOldCounter = 0;
	...
	sgptDataTables->pItemsLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	pWeapons = (struct D2ItemsTxt*)DATATBLS_CompileTxt(hArchive, "weapons", pTbl, &sgptDataTables->pItemDataTables.nWeaponsTxtRecordCount, sizeof(struct D2ItemsTxt));
	pArmor = (struct D2ItemsTxt*)DATATBLS_CompileTxt(hArchive, "armor", pTbl, &sgptDataTables->pItemDataTables.nArmorTxtRecordCount, sizeof(struct D2ItemsTxt));
	pMisc = (struct D2ItemsTxt*)DATATBLS_CompileTxt(hArchive, "misc", pTbl, &sgptDataTables->pItemDataTables.nMiscTxtRecordCount, sizeof(struct D2ItemsTxt));
	...
	pItems = (struct D2ItemsTxt*)D2_ALLOC_POOL(nullptr, sizeof(struct D2ItemsTxt) * sgptDataTables->pItemDataTables.nItemsTxtRecordCount);
	...
	memcpy(sgptDataTables->pItemDataTables.pWeapons, pWeapons, sizeof(struct D2ItemsTxt) * sgptDataTables->pItemDataTables.nWeaponsTxtRecordCount);
	memcpy(sgptDataTables->pItemDataTables.pArmor, pArmor, sizeof(struct D2ItemsTxt) * sgptDataTables->pItemDataTables.nArmorTxtRecordCount);
	memcpy(sgptDataTables->pItemDataTables.pMisc, pMisc, sizeof(struct D2ItemsTxt) * sgptDataTables->pItemDataTables.nMiscTxtRecordCount);
...
void __fastcall DATATBLS_LoadMagicSuffix_Prefix_AutomagicTxt(HD2ARCHIVE hArchive)
{
	...
	struct D2MagicAffixTxt* pMagicSuffix = NULL;
	struct D2MagicAffixTxt* pMagicPrefix = NULL;
	struct D2MagicAffixTxt* pAutoMagic = NULL;
	...
	pMagicSuffix = (struct D2MagicAffixTxt*)DATATBLS_CompileTxt(hArchive, "magicsuffix", pTbl, &nSuffixRecords, sizeof(struct D2MagicAffixTxt));
	pMagicPrefix = (struct D2MagicAffixTxt*)DATATBLS_CompileTxt(hArchive, "magicprefix", pTbl, &nPrefixRecords, sizeof(struct D2MagicAffixTxt));
	pAutoMagic = (struct D2MagicAffixTxt*)DATATBLS_CompileTxt(hArchive, "automagic", pTbl, &nAutoMagicRecords, sizeof(struct D2MagicAffixTxt));

	sgptDataTables->pMagicAffixDataTables.nMagicAffixTxtRecordCount = nSuffixRecords + nPrefixRecords + nAutoMagicRecords;
	sgptDataTables->pMagicAffixDataTables.pMagicAffixTxt = (struct D2MagicAffixTxt *)D2_ALLOC_POOL(nullptr, sizeof(struct D2MagicAffixTxt) * sgptDataTables->pMagicAffixDataTables.nMagicAffixTxtRecordCount);
	// Suffixes
	sgptDataTables->pMagicAffixDataTables.pMagicSuffix = sgptDataTables->pMagicAffixDataTables.pMagicAffixTxt;
	memcpy(sgptDataTables->pMagicAffixDataTables.pMagicAffixTxt, pMagicSuffix, sizeof(struct D2MagicAffixTxt) * nSuffixRecords);
	// Prefixes
	sgptDataTables->pMagicAffixDataTables.pMagicPrefix = &sgptDataTables->pMagicAffixDataTables.pMagicSuffix[nSuffixRecords];
	memcpy(&sgptDataTables->pMagicAffixDataTables.pMagicSuffix[nSuffixRecords], pMagicPrefix, sizeof(struct D2MagicAffixTxt) * nPrefixRecords);
	// Automagic
	sgptDataTables->pMagicAffixDataTables.pAutoMagic = &sgptDataTables->pMagicAffixDataTables.pMagicPrefix[nPrefixRecords];
	memcpy(&sgptDataTables->pMagicAffixDataTables.pMagicPrefix[nPrefixRecords], pAutoMagic, sizeof(struct D2MagicAffixTxt) * nAutoMagicRecords);
...
void __fastcall DATATBLS_LoadRareSuffix_PrefixTxt(HD2ARCHIVE hArchive)
{
	...
	struct D2RareAffixTxt* pRareSuffix = NULL;
	struct D2RareAffixTxt* pRarePrefix = NULL;
	...
	pRareSuffix = (struct D2RareAffixTxt*)DATATBLS_CompileTxt(hArchive, "raresuffix", pTbl, &nSuffixRecords, sizeof(struct D2RareAffixTxt));
	pRarePrefix = (struct D2RareAffixTxt*)DATATBLS_CompileTxt(hArchive, "rareprefix", pTbl, &nPrefixRecords, sizeof(struct D2RareAffixTxt));

	sgptDataTables->pRareAffixDataTables.nRareAffixTxtRecordCount = nPrefixRecords + nSuffixRecords;

	sgptDataTables->pRareAffixDataTables.pRareAffixTxt = (struct D2RareAffixTxt*)D2_ALLOC_POOL(nullptr, sizeof(struct D2RareAffixTxt) * (nPrefixRecords + nSuffixRecords));

	sgptDataTables->pRareAffixDataTables.pRareSuffix = sgptDataTables->pRareAffixDataTables.pRareAffixTxt;
	memcpy(sgptDataTables->pRareAffixDataTables.pRareAffixTxt, pRareSuffix, sizeof(struct D2RareAffixTxt) * nSuffixRecords);
	sgptDataTables->pRareAffixDataTables.pRarePrefix = &sgptDataTables->pRareAffixDataTables.pRareSuffix[nSuffixRecords];
	memcpy(&sgptDataTables->pRareAffixDataTables.pRareSuffix[nSuffixRecords], pRarePrefix, sizeof(struct D2RareAffixTxt) * nPrefixRecords);
...
void __fastcall DATATBLS_LoadUniqueItemsTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pUniqueItemsLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pUniqueItemsTxt = (struct D2UniqueItemsTxt*)DATATBLS_CompileTxt(hArchive, "uniqueitems", pTbl, &sgptDataTables->nUniqueItemsTxtRecordCount, sizeof(struct D2UniqueItemsTxt));
...
void __fastcall DATATBLS_LoadSets_SetItemsTxt(HD2ARCHIVE hArchive)
{
	...
	struct D2BinField pSetItemsTbl[] =
	{
	...
	sgptDataTables->pSetsLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pSetsTxt = (struct D2SetsTxt*)DATATBLS_CompileTxt(hArchive, "sets", pSetsTbl, &sgptDataTables->nSetsTxtRecordCount, sizeof(struct D2SetsTxt));
	...
	sgptDataTables->pSetItemsLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pSetItemsTxt = (struct D2SetItemsTxt*)DATATBLS_CompileTxt(hArchive, "setitems", pSetItemsTbl, &sgptDataTables->nSetItemsTxtRecordCount, sizeof(struct D2SetItemsTxt));
...
void __fastcall DATATBLS_LoadQualityItemsTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pQualityItemDataTables.pQualityItemsTxt = (struct D2QualityItemsTxt*)DATATBLS_CompileTxt(hArchive, "qualityitems", pTbl, &sgptDataTables->pQualityItemDataTables.nQualityItemsTxtRecordCount, sizeof(struct D2QualityItemsTxt));
...
void __fastcall DATATBLS_LoadGemsTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pGemDataTables.pGemsTxt = (struct D2GemsTxt*)DATATBLS_CompileTxt(hArchive, "gems", pTbl, &sgptDataTables->pGemDataTables.nGemsTxtRecordCount, sizeof(struct D2GemsTxt));
...
void __fastcall DATATBLS_LoadBooksTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pBookDataTables.pBooksTxt = (struct D2BooksTxt*)DATATBLS_CompileTxt(hArchive, "books", pTbl, &sgptDataTables->pBookDataTables.nBooksTxtRecordCount, sizeof(struct D2BooksTxt));
}
...
void __fastcall DATATBLS_LoadLowQualityItemsTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pLowQualityItemDataTables.pLowQualityItemsTxt = (struct D2LowQualityItemsTxt *)DATATBLS_CompileTxt(hArchive, "lowqualityitems", pTbl, &sgptDataTables->pLowQualityItemDataTables.nLowQualityItemsTxtRecordCount, sizeof(struct D2LowQualityItemsTxt));
...
void __fastcall DATATBLS_LoadItemRatioTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pItemRatioDataTables.pItemRatioTxt = (struct D2ItemRatioTxt*)DATATBLS_CompileTxt(hArchive, "itemratio", pTbl, &sgptDataTables->pItemRatioDataTables.nItemRatioTxtRecordCount, sizeof(struct D2ItemRatioTxt));
}
...
{
	struct D2ItemStatCostDesc* pElem1 = (struct D2ItemStatCostDesc*)pRecord1;
	struct D2ItemStatCostDesc* pElem2 = (struct D2ItemStatCostDesc*)pRecord2;
...
void __fastcall DATATBLS_LoadItemStatCostTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pItemStatCostLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pItemStatCostTxt = (struct D2ItemStatCostTxt*)DATATBLS_CompileTxt(hArchive, "itemstatcost", pTbl, &sgptDataTables->nItemStatCostTxtRecordCount, sizeof(struct D2ItemStatCostTxt));
...
#define MAX_STATS 511
	...
	{
		struct D2ItemStatCostTxt* pCurrentStatRecord = &sgptDataTables->pItemStatCostTxt[nStatId];
		uint8_t nCurrentStatOp = pCurrentStatRecord->nOp;
		if (nCurrentStatOp <= 0 || nCurrentStatOp >= 14)
			continue;

		nOpBase = pCurrentStatRecord->wOpBase;
		...
		if (nOpBase < sgptDataTables->nItemStatCostTxtRecordCount)
		{
			...
			{
				pCurrentStatRecord->bHasOpApplyingToItem = TRUE;
			}
		}

		for (int i = 0; i < ARRAY_SIZE(pCurrentStatRecord->wOpStat); i++)
		{
			const uint16_t wOpStat = pCurrentStatRecord->wOpStat[i];
			...
			pCurrentStatRecord->bIsBaseOfOtherStatOp = TRUE;

			int nNextFreeId = 0;
			while (nNextFreeId < 16)
			{
				struct D2ItemStatCostTxt* pCurrentOpStatRecord = &sgptDataTables->pItemStatCostTxt[wOpStat];
				// Find first unused slot
				if (pCurrentOpStatRecord->pOpStatData[nNextFreeId].nOp == STAT_OP_NONE)
				{
					pCurrentOpStatRecord->pOpStatData[nNextFreeId].nStat = nStatId;
					pCurrentOpStatRecord->pOpStatData[nNextFreeId].nOp = nCurrentStatOp;
					pCurrentOpStatRecord->pOpStatData[nNextFreeId].nOpBase = nOpBase;
					pCurrentOpStatRecord->pOpStatData[nNextFreeId].nOpParam = pCurrentStatRecord->nOpParam;

					pCurrentOpStatRecord->bHasOpStatData = TRUE;

					if (nStatId == STAT_MAXHP || wOpStat == STAT_MAXHP)
					{
						pCurrentStatRecord->dwItemStatFlags |= gdwBitMasks[ITEMSTATCOSTFLAGINDEX_HP];
						pCurrentStatRecord->dwItemStatFlags |= gdwBitMasks[ITEMSTATCOSTFLAGINDEX_HP_MANA_STAMINA];
					}
					else if (nStatId == STAT_MAXMANA || wOpStat == STAT_MAXMANA)
					{
						pCurrentStatRecord->dwItemStatFlags |= gdwBitMasks[ITEMSTATCOSTFLAGINDEX_MANA];
						pCurrentStatRecord->dwItemStatFlags |= gdwBitMasks[ITEMSTATCOSTFLAGINDEX_HP_MANA_STAMINA];
					}
					else if (nStatId == STAT_MAXSTAMINA || wOpStat == STAT_MAXSTAMINA)
					{
						pCurrentStatRecord->dwItemStatFlags |= gdwBitMasks[ITEMSTATCOSTFLAGINDEX_STAMINA];
						pCurrentStatRecord->dwItemStatFlags |= gdwBitMasks[ITEMSTATCOSTFLAGINDEX_HP_MANA_STAMINA];
					}
					break;
				}
	...
	qsort(pStatsWithDescFunc, nStatsWithDescFunc, sizeof(struct D2ItemStatCostDesc), DATATBLS_CompareItemStatCostDescs);
...
void __fastcall DATATBLS_LoadPropertiesTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pPropertiesLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pPropertiesTxt = (struct D2PropertiesTxt*)DATATBLS_CompileTxt(hArchive, "properties", pTbl, &sgptDataTables->nPropertiesTxtRecordCount, sizeof(struct D2PropertiesTxt));
}
...
void __fastcall DATATBLS_LoadGambleTxt(HD2ARCHIVE hArchive)
{
	int nCounter = 0;
	int nItemId = 0;
	struct D2GambleTxt* pGambleTxt = NULL;
	...
	pGambleTxt = (struct D2GambleTxt*)DATATBLS_CompileTxt(hArchive, "gamble", pTbl, &sgptDataTables->pGambleDataTables.nGambleTxtRecordCount, sizeof(struct D2GambleTxt));
	...
	if (sgptDataTables->pGambleDataTables.nGambleTxtRecordCount)
	{
		...
		qsort(pGambleTxt, sgptDataTables->pGambleDataTables.nGambleTxtRecordCount, sizeof(struct D2GambleTxt), DATATBLS_CompareGambleTxtRecords);
	}
	else
	{
...
{
	struct D2GambleTxt* pElem1 = (struct D2GambleTxt*)pRecord1;
	struct D2GambleTxt* pElem2 = (struct D2GambleTxt*)pRecord2;
...
BOOL __fastcall DATATBLS_CheckItemTypesEquivalenceNested(int nItemType1, int nItemType2)
{
	...
	if (nItemType1 > 0 && nItemType1 < sgptDataTables->nItemTypesTxtRecordCount)
	{
		...
		while (1)
		{
			...
			struct D2ItemTypesTxt* pItemTypesTxtRecord = &sgptDataTables->pItemTypesTxt[nItemType];
...
void __fastcall DATATBLS_LoadItemTypesTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pItemTypesLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pItemTypesTxt = (struct D2ItemTypesTxt*)DATATBLS_CompileTxt(hArchive, "itemtypes", pTbl, &sgptDataTables->nItemTypesTxtRecordCount, sizeof(struct D2ItemTypesTxt));
...
void __fastcall DATATBLS_LoadRunesTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pRunesLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	...
	sgptDataTables->pRuneDataTables.pRunesTxt = (struct D2RunesTxt*)DATATBLS_CompileTxt(hArchive, "runes", pTbl, &sgptDataTables->pRuneDataTables.nRunesTxtRecordCount, sizeof(struct D2RunesTxt));
...
void __stdcall DATATBLS_AddOrChangeRunesTxtRecord(int nRecordId, struct D2RunesTxt* pRecord)
{
	...
	if (nRecordId >= sgptDataTables->pRuneDataTables.nRunesTxtRecordCount)
	{
		...
		nSize = sizeof(struct D2RunesTxt) * (nRecordId + 1);

		if (DATATBLS_LoadFromBin)
		{
			pRunesTxt = (struct D2RunesTxt*)((char*)sgptDataTables->pRuneDataTables.pRunesTxt - 4);
			nSize += 4;
		}

		pTmp = (struct D2RunesTxt*)D2_REALLOC_POOL(NULL, pRunesTxt, nSize);

		if (DATATBLS_LoadFromBin)
		{
			*(uint32_t*)pTmp = nRecordId + 1;
			pTmp = (struct D2RunesTxt*)((char*)pTmp + 4);
		}
		sgptDataTables->pRuneDataTables.pRunesTxt = pTmp;

		memset(&pTmp[sgptDataTables->pRuneDataTables.nRunesTxtRecordCount], 0x00, sizeof(struct D2RunesTxt) * (nRecordId - sgptDataTables->pRuneDataTables.nRunesTxtRecordCount));
