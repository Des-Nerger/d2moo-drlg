...
#define gpAutomapSeed (&sgptDataTables->pAutomapSeed)
...
void __fastcall DATATBLS_LoadLevelsTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pLevelsTxt = (struct D2LevelsTxt*)DATATBLS_CompileTxt(hArchive, "levels", pTbl, &sgptDataTables->nLevelsTxtRecordCount, sizeof(struct D2LevelsTxt));
...
#define MAX_LEVEL_ID 1024
	...
	for (int i = 0; i < sgptDataTables->nLevelsTxtRecordCount; ++i)
	{
		...
		Unicode_strncpy(sgptDataTables->pLevelsTxt[i].wszLevelName, D2LANG_GetStringByReferenceString(sgptDataTables->pLevelsTxt[i].szLevelName), sizeof(sgptDataTables->pLevelsTxt[i].szLevelName));
		sgptDataTables->pLevelsTxt[i].wszLevelName[39] = (struct Unicode){0};

		Unicode_strncpy(sgptDataTables->pLevelsTxt[i].wszLevelWarp, D2LANG_GetStringByReferenceString(sgptDataTables->pLevelsTxt[i].szLevelWarp), sizeof(sgptDataTables->pLevelsTxt[i].szLevelWarp));
		sgptDataTables->pLevelsTxt[i].wszLevelWarp[39] = (struct Unicode){0};
...
void __fastcall DATATBLS_LoadLevelDefsBin(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pLevelDefBin = (struct D2LevelDefBin*)DATATBLS_CompileTxt(hArchive, "leveldefs", pTbl, 0, sizeof(struct D2LevelDefBin));
...
{	
	...
	sgptDataTables->pLvlTypesTxt = (struct D2LvlTypesTxt*)DATATBLS_CompileTxt(hArchive, "lvltypes", pTbl, &sgptDataTables->nLvlTypesTxtRecordCount, sizeof(struct D2LvlTypesTxt));
...
void __fastcall DATATBLS_LoadLvlPrestTxt(HD2ARCHIVE hArchive, int a2)
{
	...
	sgptDataTables->pLvlPrestTxt = (struct D2LvlPrestTxt*)DATATBLS_CompileTxt(hArchive, "lvlprest", pTbl, &sgptDataTables->nLvlPrestTxtRecordCount, sizeof(struct D2LvlPrestTxt));

	if (a2)
	{
		sgptDataTables->ppLvlPrestFiles = (struct D2DrlgFile**)D2_CALLOC(6 * sizeof(struct D2DrlgFile*) * sgptDataTables->nLvlPrestTxtRecordCount);
	}
		...
		if (FOG_IsExpansion() || !sgptDataTables->pLvlPrestTxt[i].dwExpansion)
		{
			for (int j = 0; j < 6; ++j)
			{
				...
				if (nLength > 1)
				{
					...
					strcpy_s(
						sgptDataTables->pLvlPrestTxt[i].szFile[j],
						ARRAY_SIZE(sgptDataTables->pLvlPrestTxt[i].szFile[j]),
						szBuffer);
				}
...
void __fastcall DATATBLS_LoadLvlWarpTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pLvlWarpTxt = (struct D2LvlWarpTxt*)DATATBLS_CompileTxt(hArchive, "lvlwarp", pTbl, &sgptDataTables->nLvlWarpTxtRecordCount, sizeof(struct D2LvlWarpTxt));
}
...
void __fastcall DATATBLS_LoadLvlMazeTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pLvlMazeTxt = (struct D2LvlMazeTxt*)DATATBLS_CompileTxt(hArchive, "lvlmaze", pTbl, &sgptDataTables->nLvlMazeTxtRecordCount, sizeof(struct D2LvlMazeTxt));
}
...
void __fastcall DATATBLS_LoadLvlSubTxt(HD2ARCHIVE hArchive, int a2, int a3)
{
	...
	sgptDataTables->pLvlSubTxt = (struct D2LvlSubTxt*)DATATBLS_CompileTxt(hArchive, "lvlsub", pTbl, &sgptDataTables->nLvlSubTxtRecordCount, sizeof(struct D2LvlSubTxt));
	...
	if (a2 || a3)
	{
		sgptDataTables->ppLvlSubTypeFiles = (struct D2DrlgFile**)D2_CALLOC(sizeof(struct D2DrlgFile*) * sgptDataTables->nLvlSubTxtRecordCount);
	}
	...
	for (int i = 0; i < sgptDataTables->nLvlSubTxtRecordCount; ++i)
	{
		...
		if (nLength > 1)
		{
			...
			strcpy_s(
				sgptDataTables->pLvlSubTxt[i].szFile,
				ARRAY_SIZE(sgptDataTables->pLvlSubTxt[i].szFile),
				szBuffer);
		}
...
void __fastcall DATATBLS_AllocGlobalTileLibraryHash()
{
	...
	struct D2TileLibraryHash** ppTileLibraryHash = NULL;
	bool bValidFileName = false;

	D2_ASSERT(sgptDataTables->pLvlTypesTxt);

	sgptDataTables->ppTileLibraryHash = (struct D2TileLibraryHash**)D2_CALLOC_POOL(nullptr, sizeof(struct D2TileLibraryHash*[1024]) * sgptDataTables->nLvlTypesTxtRecordCount);
...
void __fastcall DATATBLS_LoadAutomapTxt(HD2ARCHIVE hArchive)
{
	...
	pAutomapTxt = (struct D2AutomapTxt*)DATATBLS_CompileTxt(hArchive, "automap", pTbl, &nAutomapTxtRecordCount, sizeof(struct D2AutomapTxt));

	pAutomap = (struct D2AutomapShort*)D2_CALLOC_POOL(nullptr, sizeof(struct D2AutomapShort) * nAutomapTxtRecordCount);
