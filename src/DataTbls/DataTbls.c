...
// D2Common.0x6FDE9600
struct D2ArenaTxt* gpArenaTxtTable;
// D2Common.0x6FDE95F8
struct D2CharTemplateTxt* gpCharTemplateTxtTable;
int gnCharTemplateTxtTableRecordCount;
uint32_t gnCharTemplateStartIds[64];
// D2Common.0x6FDE9604
struct D2BeltsTxt* gpBeltsTxtTable;
// D2Common.0x6FDE9608
struct D2DataTables gpDataTables;
// D2Common.0x6FDD6A20 (#10042)
struct D2DataTables * sgptDataTables = &gpDataTables;
...
void __fastcall DATATBLS_InitUnicodeClassNamesInCharStatsTxt()
{
	...
	for (int i = 0; i < sgptDataTables->nCharStatsTxtRecordCount; ++i)
	{
		...
		const struct Unicode* wszClassName = D2LANG_GetStringByReferenceString(sgptDataTables->pCharStatsTxt[i].szClassName);

		if (wszClassName)
		{
			Unicode_strncpy(sgptDataTables->pCharStatsTxt[i].wszClassName, wszClassName, ARRAY_SIZE(sgptDataTables->pCharStatsTxt[i].wszClassName));
		}
		else
		{
			Unicode_toUnicode(wszName, sgptDataTables->pCharStatsTxt[i].szClassName, ARRAY_SIZE(sgptDataTables->pCharStatsTxt[i].szClassName) - 1);
			Unicode_strcpy(wszClass, (const struct Unicode*)L"<");
			Unicode_strcat(wszClass, wszName);
			Unicode_strcat(wszClass, (const struct Unicode*)L">");

			Unicode_strncpy(sgptDataTables->pCharStatsTxt[i].wszClassName, wszClass, ARRAY_SIZE(sgptDataTables->pCharStatsTxt[i].wszClassName));
		}
	}
}
...
void __fastcall DATATBLS_LoadStatesTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pStatesLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pStatesTxt = (struct D2StatesTxt*)DATATBLS_CompileTxt(hArchive, "states", pTbl, &sgptDataTables->nStatesTxtRecordCount, sizeof(struct D2StatesTxt));
...
void __fastcall DATATBLS_LoadPetTypeTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pPetTypeLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pPetTypeTxt = (struct D2PetTypeTxt*)DATATBLS_CompileTxt(hArchive, "pettype", pTbl, &sgptDataTables->nPetTypeTxtRecordCount, sizeof(struct D2PetTypeTxt));
...
char* __stdcall DATATBLS_GetUnitNameFromUnitTypeAndClassId(int nUnitType, int nClassId, char* szName)
{
	...
	struct D2ObjectsTxt* pObjectsTxtRecord = NULL;
	...
	*szName = 0;
	...
	case UNIT_MONSTER:
		...
		{
			Unicode_unicode2Win(szName, (const struct Unicode*)D2LANG_GetStringFromTblIndex(pMonStatsTxtRecord->wNameStr), 64);
		}
		else
		{
	...
	case UNIT_ITEM:
		...
		{
			Unicode_unicode2Win(szName, (const struct Unicode*)D2LANG_GetStringFromTblIndex(pItemsTxtRecord->wNameStr), 64);
		}
		else
		{
...
void __stdcall DATATBLS_LoadAllTxts(HD2ARCHIVE hArchive, int a2, int a3)
{
	...
	sgptDataTables->pExperienceTxt = (struct D2ExperienceDataTbl*)DATATBLS_CompileTxt(hArchive, "experience", pTbl, NULL, sizeof(struct D2ExperienceTxt));
...
void __fastcall DATATBLS_LoadSomeTxts(HD2ARCHIVE hArchive)
{
	...
	struct D2BinField pHireDescTbl[] =
	{
		{ "code", TXTFIELD_ASCIITOCODE, 0, 0, &sgptDataTables->pHireDescLinker },
		{ "end", TXTFIELD_NONE, 0, 0, NULL },
	};
	struct D2BinField pMonModeTbl[] =
	{
		{ "code", TXTFIELD_ASCIITOCODE, 0, 0, &sgptDataTables->pMonModeLinker },
		{ "end", TXTFIELD_NONE, 0, 0, NULL },
	};
	struct D2BinField pPlayerClassTbl[] =
	{
		{ "code", TXTFIELD_ASCIITOCODE, 0, 0, &sgptDataTables->pPlayerClassLinker },
		{ "end", TXTFIELD_NONE, 0, 0, NULL },
	};
	struct D2BinField pPlrModeTbl[] =
	{
		{ "code", TXTFIELD_ASCIITOCODE, 0, 0, &sgptDataTables->pPlrModeLinker },
		{ "end", TXTFIELD_NONE, 0, 0, NULL },
	};
	struct D2BinField pStorePageTbl[] =
	{
		{ "code", TXTFIELD_ASCIITOCODE, 0, 0, &sgptDataTables->pStorePageLinker },
		{ "end", TXTFIELD_NONE, 0, 0, NULL },
	};
	struct D2BinField pMonAiTbl[] =
	{
		{ "AI", TXTFIELD_NAMETOINDEX, 0, 0, &sgptDataTables->pMonAiLinker },
		{ "end", TXTFIELD_NONE, 0, 0, NULL },
	};
	struct D2BinField pHitClassTbl[] =
	{
		{ "code", TXTFIELD_ASCIITOCODE, 0, 0, &sgptDataTables->pHitClassLinker },
		{ "end", TXTFIELD_NONE, 0, 0, NULL },
	};
	struct D2BinField pMonPlaceTbl[] =
	{
		{ "code", TXTFIELD_NAMETOINDEX, 0, 0, &sgptDataTables->pMonPlaceLinker },
		{ "end", TXTFIELD_NONE, 0, 0, NULL },
	};
	struct D2BinField pCompCodeTbl[] =
	{
		{ "code", TXTFIELD_ASCIITOCODE, 0, 0, &sgptDataTables->pCompCodeLinker },
		{ "end", TXTFIELD_NONE, 0, 0, NULL },
	};
	struct D2BinField pSkillCalcTbl[] =
	{
		{ "code", TXTFIELD_ASCIITOCODE, 0, 0, &sgptDataTables->pSkillCalcLinker },
		{ "end", TXTFIELD_NONE, 0, 0, NULL },
	};
	struct D2BinField pElemTypesTbl[] =
	{
		{ "code", TXTFIELD_ASCIITOCODE, 0, 0, &sgptDataTables->pElemTypesLinker },
		{ "end", TXTFIELD_NONE, 0, 0, NULL },
	};
	struct D2BinField pMissCalcTbl[] =
	{
		{ "code", TXTFIELD_ASCIITOCODE, 0, 0, &sgptDataTables->pMissileCalcLinker },
		{ "end", TXTFIELD_NONE, 0, 0, NULL },
	};
	struct D2BinField pBodyLocsTbl[] =
	{
		{ "code", TXTFIELD_ASCIITOCODE, 0, 0, &sgptDataTables->pBodyLocsLinker },
		{ "end", TXTFIELD_NONE, 0, 0, NULL },
	};
	struct D2BinField pSkillCodeTbl[] =
	{
		{ "skill", TXTFIELD_NAMETOINDEX, 0, 0, &sgptDataTables->iSkillCode },
		{ "end", TXTFIELD_NONE, 0, 0, NULL },
	};
	struct D2BinField pColorsTbl[] =
	{
		{ "code", TXTFIELD_ASCIITOCODE, 0, 0, &sgptDataTables->pColorsLinker },
		{ "end", TXTFIELD_NONE, 0, 0, NULL },
	};
	struct D2BinField pEventsTbl[] =
	{
		{ "event", TXTFIELD_NAMETOINDEX, 0, 0, &sgptDataTables->pEventsLinker },
		{ "end", TXTFIELD_NONE, 0, 0, NULL },
	};

	sgptDataTables->pCompCodeLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pCompCodeTxt = (struct D2CompCodeTxt*)DATATBLS_CompileTxt(hArchive, "compcode", pCompCodeTbl, &sgptDataTables->nCompCodeTxtRecordCount, sizeof(struct D2CompCodeTxt));

	if (sgptDataTables->bCompileTxt)
	{
		sgptDataTables->pPlayerClassLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		sgptDataTables->pPlayerClassTxt = (struct D2PlayerClassTxt*)DATATBLS_CompileTxt(hArchive, "playerclass", pPlayerClassTbl, &nRecordCount, sizeof(struct D2PlayerClassTxt));

		sgptDataTables->pBodyLocsLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		sgptDataTables->pBodyLocsTxt = (struct D2BodyLocsTxt*)DATATBLS_CompileTxt(hArchive, "bodylocs", pBodyLocsTbl, &nRecordCount, sizeof(struct D2BodyLocsTxt));

		sgptDataTables->pStorePageLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		sgptDataTables->pStorePageTxt = (struct D2StorePageTxt*)DATATBLS_CompileTxt(hArchive, "storepage", pStorePageTbl, &nRecordCount, sizeof(struct D2StorePageTxt));

		sgptDataTables->pElemTypesLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		sgptDataTables->pElemTypesTxt = (struct D2ElemTypesTxt*)DATATBLS_CompileTxt(hArchive, "elemtypes", pElemTypesTbl, &nRecordCount, sizeof(struct D2ElemTypesTxt));

		sgptDataTables->pHitClassLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		sgptDataTables->pHitClassTxt = (struct D2HitClassTxt*)DATATBLS_CompileTxt(hArchive, "hitclass", pHitClassTbl, &nRecordCount, sizeof(struct D2HitClassTxt));

		sgptDataTables->pColorsLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		sgptDataTables->pColorsTxt = (struct D2ColorsTxt*)DATATBLS_CompileTxt(hArchive, "colors", pColorsTbl, &nRecordCount, sizeof(struct D2ColorsTxt));

		sgptDataTables->pHireDescLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		sgptDataTables->pHireDescTxt = (struct D2HireDescTxt*)DATATBLS_CompileTxt(hArchive, "hiredesc", pHireDescTbl, &nRecordCount, sizeof(struct D2HireDescTxt));

		sgptDataTables->pMonModeLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		sgptDataTables->pMonModeTxtStub = (struct D2MonModeTxtStub*)DATATBLS_CompileTxt(hArchive, "monmode", pMonModeTbl, &nRecordCount, sizeof(struct D2MonModeTxtStub));

		sgptDataTables->pPlrModeLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		sgptDataTables->pPlrModeTxtStub = (struct D2PlrModeTxtStub*)DATATBLS_CompileTxt(hArchive, "plrmode", pPlrModeTbl, &nRecordCount, sizeof(struct D2PlrModeTxtStub));

		sgptDataTables->pMonAiLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		sgptDataTables->pMonAiTxt = (struct D2MonAiTxt*)DATATBLS_CompileTxt(hArchive, "monai", pMonAiTbl, &sgptDataTables->nMonAiTxtRecordCount, sizeof(struct D2MonAiTxt));

		sgptDataTables->pMonPlaceLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		sgptDataTables->pMonPlaceTxt = (struct D2MonPlaceTxt*)DATATBLS_CompileTxt(hArchive, "monplace", pMonPlaceTbl, &sgptDataTables->nMonPlaceTxtRecordCount, sizeof(struct D2MonPlaceTxt));

		sgptDataTables->pSkillCalcLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		sgptDataTables->pSkillCalcTxt = (struct D2SkillCalcTxt*)DATATBLS_CompileTxt(hArchive, "skillcalc", pSkillCalcTbl, &nRecordCount, sizeof(struct D2SkillCalcTxt));

		sgptDataTables->pMissileCalcLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		sgptDataTables->pMissileCalcTxt = (struct D2MissCalcTxt*)DATATBLS_CompileTxt(hArchive, "misscalc", pMissCalcTbl, &nRecordCount, sizeof(struct D2MissCalcTxt));

		sgptDataTables->iSkillCode = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		sgptDataTables->pSkillCode = (const char*)DATATBLS_CompileTxt(hArchive, "skills", pSkillCodeTbl, &nRecordCount, 2);

		sgptDataTables->pEventsLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		sgptDataTables->pEventsTxt = (struct D2EventsTxt*)DATATBLS_CompileTxt(hArchive, "events", pEventsTbl, &nRecordCount, sizeof(struct D2EventsTxt));
	}
}
...
void __fastcall DATATBLS_LoadCharStatsTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pCharStatsTxt = (struct D2CharStatsTxt*)DATATBLS_CompileTxt(hArchive, "charstats", pTbl, &sgptDataTables->nCharStatsTxtRecordCount, sizeof(struct D2CharStatsTxt));
...
void __fastcall DATATBLS_LoadDifficultyLevelsTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pDifficultyLevelsTxt = (struct D2DifficultyLevelsTxt*)DATATBLS_CompileTxt(hArchive, "difficultylevels", pTbl, &sgptDataTables->nDifficultyLevelsTxtRecordCount, sizeof(struct D2DifficultyLevelsTxt));
