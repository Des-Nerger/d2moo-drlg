...
void __cdecl DATATBLS_MonStatsSkillModeLinker(char* pSrc, void* pRecord, int nOffset, int nPosition, int nTxtRow, int nTxtColumn)
{
	...
	pMonStatsTxtRecord = (struct D2MonStatsTxt*)pRecord;
...
void __cdecl DATATBLS_LoadMonStatsTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pMonStatsLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pMonStatsTxt = (struct D2MonStatsTxt*)DATATBLS_CompileTxt(hArchive, "monstats", pTbl, &sgptDataTables->nMonStatsTxtRecordCount, sizeof(struct D2MonStatsTxt));
...
const struct Unicode* __cdecl DATATBLS_RollRandomUniqueTitleString(struct D2Unit* pUnit)
{
	...
	if (pUnit && sgptDataTables->nUniqueTitleTxtRecordCount > 0)
	{
		...
		{
			const struct Unicode* pString = D2LANG_GetStringFromTblIndex(sgptDataTables->pUniqueTitleTxt[nRand].wStringId);
			if (pString)
			{
				return pString;
			}
...
const struct Unicode* __cdecl DATATBLS_RollRandomUniquePrefixString(struct D2Unit* pUnit)
{
	...
	if (pUnit && sgptDataTables->nUniquePrefixTxtRecordCount > 0)
	{
		...
		{
			const struct Unicode* pString = D2LANG_GetStringFromTblIndex(sgptDataTables->pUniquePrefixTxt[nRand].wStringId);
			if (pString)
			{
				return pString;
			}
...
const struct Unicode* __cdecl DATATBLS_RollRandomUniqueSuffixString(struct D2Unit* pUnit)
{
	...
	if (pUnit && sgptDataTables->nUniqueSuffixTxtRecordCount > 0)
	{
		...
		{
			const struct Unicode* pString = D2LANG_GetStringFromTblIndex(sgptDataTables->pUniqueSuffixTxt[nRand].wStringId);
			if (pString)
			{
				return pString;
			}
...
const struct Unicode* __cdecl DATATBLS_RollRandomUniqueAppellationString(struct D2Unit* pUnit)
{
	...
	if (pUnit && sgptDataTables->nUniqueAppellationTxtRecordCount > 0)
	{
		...
		{
			const struct Unicode* pString = D2LANG_GetStringFromTblIndex(sgptDataTables->pUniqueAppellationTxt[nRand].wStringId);
			if (pString)
			{
				return pString;
			}
...
struct D2TCExShort* DATATBLS_NewTreasureClassEx(const char* nTreasureClass)
{
	...
	{
		sgptDataTables->pTreasureClassEx = (struct D2TCExShort*)D2_REALLOC_POOL(NULL, sgptDataTables->pTreasureClassEx, sizeof(struct D2TCExShort) * (sgptDataTables->nTreasureClassEx + 16));
	}
...
void DATATBLS_ParseTreasureClassItem(struct D2TCExShort* pTCExTxtRecord, struct D2TreasureClassExTxt* pTreasureClassExTxt, int nLine, int nItem)
{
	...
	else
	{
		...
		{
			int nItemId = 0;
			struct D2ItemsTxt* pItemsTxtRecord =
				DATATBLS_GetItemRecordFromItemCode(DATATBLS_StringToCode(szText), &nItemId);
			if (pItemsTxtRecord)
			{
				const int nNewTypeIdx = pTCExTxtRecord->nTypes;
				DATATBLS_ReallocTCExInfo(pTCExTxtRecord, nNewTypeIdx + 1);
				struct D2TCExInfo* pTCExInfo = &pTCExTxtRecord->pInfo[nNewTypeIdx];
...
void __cdecl DATATBLS_LoadTreasureClassExTxt(HD2ARCHIVE hArchive)
{
	...
	int nRecordCount = 0;
	struct D2TreasureClassExTxt* pTreasureClassExTxt = (struct D2TreasureClassExTxt*)DATATBLS_CompileTxt(hArchive, "treasureclassex", pTbl, &nRecordCount, sizeof(struct D2TreasureClassExTxt));
	...
	for (int i = 0; i < nRecordCount; ++i)
	{
		...
		struct D2TCExShort* pTCExTxtRecord = DATATBLS_NewTreasureClassEx(pTreasureClassExTxt[i].szTreasureClass);
...
void __cdecl DATATBLS_LoadMonItemPercentTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pMonItemPercentDataTables.pMonItemPercentTxt = (struct D2MonItemPercentTxt*)DATATBLS_CompileTxt(hArchive, "monitempercent", pTbl, &sgptDataTables->pMonItemPercentDataTables.nMonItemPercentTxtRecordCount, sizeof(struct D2MonItemPercentTxt));
}
...
void __cdecl DATATBLS_LoadMonUModTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pMonUModLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pMonUModTxt = (struct D2MonUModTxt*)DATATBLS_CompileTxt(hArchive, "monumod", pTbl, &sgptDataTables->nMonUModTxtRecordCount, sizeof(struct D2MonUModTxt));
...
void __cdecl DATATBLS_LoadSuperUniquesTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pSuperUniquesLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pSuperUniquesTxt = (struct D2SuperUniquesTxt*)DATATBLS_CompileTxt(hArchive, "superuniques", pTbl, &sgptDataTables->nSuperUniquesTxtRecordCount, sizeof(struct D2SuperUniquesTxt));
...
void __cdecl DATATBLS_LoadHirelingTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pHirelingTxt = (struct D2HirelingTxt*)DATATBLS_CompileTxt(hArchive, "hireling", pTbl, &sgptDataTables->nHirelingTxtRecordCount, sizeof(struct D2HirelingTxt));
...
struct D2HirelingTxt* __cdecl DATATBLS_GetHirelingTxtRecordFromIdAndLevel(BOOL bExpansion, int nId, int nLevel)
{
	...
	struct D2HirelingTxt* pBestLevelHireling = nullptr;
	for(int nRecordIdx = nStartRecordId; nRecordIdx < sgptDataTables->nHirelingTxtRecordCount; nRecordIdx++)
	{
		struct D2HirelingTxt* pHirelingTxtRecord = &sgptDataTables->pHirelingTxt[nRecordIdx];
...
void __cdecl DATATBLS_LoadNpcTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pNpcTxt = (struct D2NpcTxt*)DATATBLS_CompileTxt(hArchive, "npc", pTbl, &sgptDataTables->nNpcTxtRecordCount, sizeof(struct D2NpcTxt));
}
...
void __cdecl DATATBLS_LoadMonSoundsTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pMonSoundsLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pMonSoundsTxt = (struct D2MonSoundsTxt*)DATATBLS_CompileTxt(hArchive, "monsounds", pTbl, &sgptDataTables->nMonSoundsTxtRecordCount, sizeof(struct D2MonSoundsTxt));
}
...
void __cdecl DATATBLS_MonStats2CompositLinker(char* pSrc, void* pRecord, int nOffset, int nPosition, int nTxtRow, int nTxtColumn)
{
	...
	pMonStats2TxtRecord = (struct D2MonStats2Txt*)pRecord;
	if (pMonStats2TxtRecord && nOffset >= 0 && nOffset < 16)
	{
		pMonStats2TxtRecord->nComponentChoiceCounts[nOffset] = 0;

		memset(&pMonStats2TxtRecord->unk0x26[nOffset], -1, sizeof(struct D2UnkMonComp));
...
void __cdecl DATATBLS_LoadMonStats2Txt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pMonStats2Linker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pMonStats2Txt = (struct D2MonStats2Txt*)DATATBLS_CompileTxt(hArchive, "monstats2", pTbl, &sgptDataTables->nMonStats2TxtRecordCount, sizeof(struct D2MonStats2Txt));
}
...
void __cdecl DATATBLS_LoadMonTypeTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pMonTypeLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pMonTypeTxt = (struct D2MonTypeTxt*)DATATBLS_CompileTxt(hArchive, "montype", pTbl, &sgptDataTables->nMonTypeTxtRecordCount, sizeof(struct D2MonTypeTxt));
...
void __cdecl DATATBLS_LoadMonPropTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pMonPropLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pMonPropTxt = (struct D2MonPropTxt*)DATATBLS_CompileTxt(hArchive, "monprop", pTbl, &sgptDataTables->nMonPropTxtRecordCount, sizeof(struct D2MonPropTxt));
}
...
void __cdecl DATATBLS_LoadMonLvlTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pMonLvlTxt = (struct D2MonLvlTxt*)DATATBLS_CompileTxt(hArchive, "monlvl", pTbl, &sgptDataTables->nMonLvlTxtRecordCount, sizeof(struct D2MonLvlTxt));
}
...
void __cdecl DATATBLS_LoadMonPresetTxt(HD2ARCHIVE hArchive)
{
	...
	pMonPresetTxt = (struct D2MonPresetTxt*)DATATBLS_CompileTxt(hArchive, "monpreset", pTbl, &nRecordCount, sizeof(struct D2MonPresetTxt));
...
void __cdecl DATATBLS_LoadSoundsTxt(HD2ARCHIVE hArchive)
{
	...
	if (sgptDataTables->bCompileTxt)
	{
		sgptDataTables->pSoundsLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		sgptDataTables->pSoundsTxtCodes = (struct D2SoundsTxtStub*)DATATBLS_CompileTxt(hArchive, "sounds", pTbl, &sgptDataTables->nSoundsTxtCodes, sizeof(struct D2SoundsTxtStub));
	}
}
...
void __cdecl DATATBLS_LoadMonEquipTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pMonEquipTxt = (struct D2MonEquipTxt*)DATATBLS_CompileTxt(hArchive, "monequip", pTbl, &sgptDataTables->nMonEquipTxtRecordCount, sizeof(struct D2MonEquipTxt));
...
void __cdecl DATATBLS_LoadSomeMonsterTxts(HD2ARCHIVE hArchive)
{
	...
	struct D2TCExShort** ppTCEx = NULL;
	int nTxtRow = 0;
	...
	sgptDataTables->pUniqueTitleTxt = (struct D2UniqueTitleTxt*)DATATBLS_CompileTxt(hArchive, "uniquetitle", pTbl, &sgptDataTables->nUniqueTitleTxtRecordCount, sizeof(struct D2UniqueTitleTxt));
	sgptDataTables->pUniquePrefixTxt = (struct D2UniquePrefixTxt*)DATATBLS_CompileTxt(hArchive, "uniqueprefix", pTbl, &sgptDataTables->nUniquePrefixTxtRecordCount, sizeof(struct D2UniquePrefixTxt));
	sgptDataTables->pUniqueSuffixTxt = (struct D2UniqueSuffixTxt*)DATATBLS_CompileTxt(hArchive, "uniquesuffix", pTbl, &sgptDataTables->nUniqueSuffixTxtRecordCount, sizeof(struct D2UniqueSuffixTxt));
	sgptDataTables->pUniqueAppellationTxt = (struct D2UniqueAppellationTxt*)DATATBLS_CompileTxt(hArchive, "uniqueappellation", pTbl, &sgptDataTables->nUniqueAppellationTxtRecordCount, sizeof(struct D2UniqueAppellationTxt));

	DATATBLS_LoadMonLvlTxt(hArchive);

	sgptDataTables->pTreasureClassExLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
