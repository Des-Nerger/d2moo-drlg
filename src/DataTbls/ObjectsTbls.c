...
void __fastcall DATATBLS_LoadObjectsTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pObjectsTxt = (struct D2ObjectsTxt*)DATATBLS_CompileTxt(hArchive, "objects", pTbl, &sgptDataTables->nObjectsTxtRecordCount, sizeof(struct D2ObjectsTxt));
	...
	for (int i = 0; i < sgptDataTables->nObjectsTxtRecordCount; ++i)
	{
		...
		Unicode_strncpy(sgptDataTables->pObjectsTxt[i].wszName, D2LANG_GetStringByReferenceString(sgptDataTables->pObjectsTxt[i].szName), sizeof(sgptDataTables->pObjectsTxt[i].szName));
...
void __fastcall DATATBLS_LoadObjGroupTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pObjGroupTxt = (struct D2ObjGroupTxt*)DATATBLS_CompileTxt(hArchive, "objgroup", pTbl, &sgptDataTables->nObjGroupTxtRecordCount, sizeof(struct D2ObjGroupTxt));
}
...
void __fastcall DATATBLS_LoadShrinesTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pShrinesTxt = (struct D2ShrinesTxt*)DATATBLS_CompileTxt(hArchive, "shrines", pTbl, &sgptDataTables->nShrinesTxtRecordCount, sizeof(struct D2ShrinesTxt));
}
