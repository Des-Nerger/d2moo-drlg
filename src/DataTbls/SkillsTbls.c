...
void __cdecl DATATBLS_SkillCalcLinker(char* pSrc, void* pRecord, int nOffset, int nPosition, int nTxtRow, int nTxtColumn)
{
	...
	int nBufferSize = 0;
	union FOGASTNode* pCode = nullptr;
			...
			if (nBufferSize > 0)
			{
				...
				else
				{
					while (1)
					{
						...
						pCode = (union FOGASTNode*)D2_REALLOC_POOL(nullptr, pCode, nNewSize);
						nSize = sgptDataTables->nSkillsCodeSize;
						nSizeEx = sgptDataTables->nSkillsCodeSizeEx;
...
void __cdecl DATATBLS_LoadSkills_SkillDescTxt(HD2ARCHIVE hArchive)
{
	...
	struct D2TxtLink* pMonStatsLinker = NULL;
	struct D2TxtLink* pRangeLinker = NULL;
	...
	struct D2BinField pSkillDescTbl[] =
	{
	...
	{
		pRangeLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		...
		pMonStatsLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);

		int nTmpRecordCount;
		pTmpMonStatsTxt = DATATBLS_CompileTxt(hArchive, "monstats", pTmpMonStatsTbl, &nTmpRecordCount, 2);

		pSkillDescLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
		pTmpSkillDescTxt = DATATBLS_CompileTxt(hArchive, "skilldesc", pTmpSkillDescTbl, &nTmpRecordCount, 2);
	}

	sgptDataTables->pSkillsLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pSkillsTxt = (struct D2SkillsTxt*)DATATBLS_CompileTxt(hArchive, "skills", pSkillTbl, &sgptDataTables->nSkillsTxtRecordCount, sizeof(struct D2SkillsTxt));
	...
	sgptDataTables->pSkillDescLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pSkillDescTxt = (struct D2SkillDescTxt*)DATATBLS_CompileTxt(hArchive, "skilldesc", pSkillDescTbl, &sgptDataTables->nSkillDescTxtRecordCount, sizeof(struct D2SkillDescTxt));
	...
	size_t dwSize;
	sgptDataTables->pSkillsCode = (union FOGASTNode*)ARCHIVE_ALLOC_BUFFER_AND_READ_FILE_TO_IT(hArchive, szFileName, &dwSize);
	...
	sgptDataTables->pSkillDescCode = (union FOGASTNode*)ARCHIVE_ALLOC_BUFFER_AND_READ_FILE_TO_IT(hArchive, szFileName, &dwSize);
