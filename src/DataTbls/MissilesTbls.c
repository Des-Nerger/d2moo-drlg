...
void __cdecl DATATBLS_LoadMissilesTxt(HD2ARCHIVE hArchive)
{
	...
	sgptDataTables->pMissilesLinker = (struct D2TxtLink*)FOG_AllocLinker(__FILE__, __LINE__);
	sgptDataTables->pMissilesTxt = (struct D2MissilesTxt*)DATATBLS_CompileTxt(hArchive, "missiles", pTbl, &sgptDataTables->nMissilesTxtRecordCount, sizeof(struct D2MissilesTxt));
