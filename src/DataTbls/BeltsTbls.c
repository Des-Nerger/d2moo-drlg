...
void __fastcall DATATBLS_LoadBeltsTxt(HD2ARCHIVE hArchive)
{
	...
	gpBeltsTxtTable = (struct D2BeltsTxt*)DATATBLS_CompileTxt(hArchive, "belts", pTbl, &nRecordCount, sizeof(struct D2BeltsTxt));
...
void __stdcall DATATBLS_GetInvRectFromBeltsTxt(int nIndex, int bHigherRes, struct D2InvRect* pInvRect, int nBoxId)
{
	...
	const struct D2InvRect* ptBoxStats = &ptBeltsTxtRecord->beltBoxes[nBoxId];
