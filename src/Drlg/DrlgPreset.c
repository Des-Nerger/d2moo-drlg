		...
		for (int i = 0; i < nUnits; ++i)
		{
			...
			case UNIT_ITEM:
				if (nVersion > 4)
				{
					...
					strncpy_s(
						szItemCode,
						ARRAY_SIZE(szItemCode),
						/*(&Source)[4 * nUnitId]*/"HDM",
						ARRAY_SIZE(szItemCode)); //TODO: Check comment
					szItemCode[3] = 0;
...
void __fastcall DRLGPRESET_LoadDrlgFile(struct D2DrlgFile** ppDrlgFile, HD2ARCHIVE hArchive, const char* szFile)
{
	...
	else
	{
		...
		*ppDrlgFile = D2_CALLOC_STRC_POOL(nullptr, struct D2DrlgFile);
		...
		strcpy_s(pLevelFile->szPath, ARRAY_SIZE(pLevelFile->szPath), szFile);
...
// Helper function
void DRLGPRESET_SpawnRiver(struct D2DrlgMap* pMazeMap, const struct D2DrlgCoord* pDrlgCoord,void* pDrlgMemPool, struct D2DrlgGrid* pDrlgGrid, int nOffsetX)
{
	int nSubtileX = pDrlgCoord->nPosX + nOffsetX + 1;
	int nSubtileY = pDrlgCoord->nPosY;
	int nEndSubtileX = 0;
	int nEndSubtileY = pDrlgCoord->nPosY + pMazeMap->pFile->nHeight;
