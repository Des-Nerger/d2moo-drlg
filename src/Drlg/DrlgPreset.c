...
// D2Common.0x6FDEA700
struct D2LevelFileList* gpLevelFilesList_6FDEA700;


// Helper functions
void DRLGPRESET_AddPresetUnitToMap(struct D2DrlgMap* pMazeMap, struct D2PresetUnit* pNewPresetUnit, BOOL bSpawned)
{
	pNewPresetUnit->pNext = pMazeMap->pPresetUnit;
	pMazeMap->pPresetUnit = pNewPresetUnit;
	pNewPresetUnit->bSpawned |= bSpawned;
}
struct D2PresetUnit* DRLGPRESET_AllocateAndAddPresetUnitToMap(struct D2DrlgRoom* pDrlgRoom, void* pMemPool, int nUnitType, int nIndex, int nMode, int nX, int nY, struct D2DrlgMap* pMazeMap, BOOL bSpawned)
{
	struct D2PresetUnit* pPresetUnit = DRLGROOM_AllocPresetUnit(pDrlgRoom, pMemPool, nUnitType, nIndex, nMode, nX, nY);
...
int DRLGPRESET_CountPresetObjectsByAct(uint8_t nAct)
{
	for (int i = 0; i < 150; ++i)
	{
...
int DRLGPRESET_GetObjectIndexFromObjPreset(uint8_t nAct, int nUnitId)
{
...
static int32_t ReadInt32(int32_t** ppData)
{
	return *((*ppData)++);
}

static void SkipInt32s(int32_t** ppData, uint32_t nbToSkip)
{
	*ppData += nbToSkip;
}
...
{
	struct D2DS1File* pDS1File = (struct D2DS1File*)ARCHIVE_READ_FILE_TO_ALLOC_BUFFER(hArchive, szFileName, NULL);
		...
		for (int i = 0; i < nUnits; ++i)
		{
			...
			case UNIT_MONSTER:
				...
				if (nVersion > 4)
				{
					...
					struct D2MonPresetTxt* pMonPresetTxtSection = DATATBLS_GetMonPresetTxtActSection(nAct, &nMonPresetRecordsCount);

					if (D2_VERIFY(pMonPresetTxtSection && nUnitId < nMonPresetRecordsCount))
					{
						const struct D2MonPresetTxt* monPresetRecord = &pMonPresetTxtSection[nUnitId];
			...
			{
				struct D2PresetUnit* pNewPresetUnit = DRLGROOM_AllocPresetUnit(NULL, NULL, nUnitType, nUnitId, nMode, nX, nY);
	...
	if (nVersion >= 12 && pDrlgFile->nSubstMethod > DRLGSUBST_NONE && pDrlgFile->nSubstMethod <= DRLGSUBST_RANDOM)
	{
		...
		pDrlgFile->pSubstGroups = (struct D2DrlgSubstGroup*)D2_ALLOC_POOL(NULL, sizeof(struct D2DrlgSubstGroup) * pDrlgFile->nSubstGroups);
	...
	if (nVersion >= 14)
	{
		...
		for (int i = 0; i < v59; ++i)
		{
			...
			if (nNodes)
			{
				...
				struct D2PresetUnit* pPresetUnit = pDrlgFile->pPresetUnit;
				while (pPresetUnit)
				{
				...
				if (pPresetUnit)
				{
					pPresetUnit->pMapAI = D2_ALLOC_STRC_POOL(nullptr, struct D2MapAI);
					pPresetUnit->pMapAI->pPosition = (struct D2MapAIPathPosition*)D2_ALLOC_POOL(nullptr, sizeof(struct D2MapAIPathPosition) * nNodes);
...
void DRLGPRESET_LoadDrlgFile(struct D2DrlgFile** ppDrlgFile, HD2ARCHIVE hArchive, const char* szFile)
{
	...
	struct D2LevelFileList* pLevelFile = gpLevelFilesList_6FDEA700;
	...
	{
		pLevelFile = D2_ALLOC_STRC_POOL(nullptr, struct D2LevelFileList);

		*ppDrlgFile = D2_CALLOC_STRC_POOL(nullptr, struct D2DrlgFile);
...
void DRLGPRESET_FreeDrlgFile(struct D2DrlgFile** ppDrlgFile)
{
	if (*ppDrlgFile)
	{
		...
		struct D2LevelFileList* pPrevious = NULL;
		struct D2LevelFileList* pCurrent = NULL;
		...
		if (pCurrent && pCurrent->nRefCount <= 0)
		{
			...
			struct D2PresetUnit* pPresetUnit = (*ppDrlgFile)->pPresetUnit;
			while (pPresetUnit)
			{
				struct D2PresetUnit* pNextPresetUnit = pPresetUnit->pNext;
...
struct D2MapAI* DRLGPRESET_CreateCopyOfMapAI(void* pMemPool, struct D2MapAI* pMapAI)
{
	struct D2MapAI* pNewMapAI = D2_ALLOC_STRC_POOL(pMemPool, struct D2MapAI);

	pNewMapAI->nPathNodes = pMapAI->nPathNodes;
	pNewMapAI->pPosition = (struct D2MapAIPathPosition*)D2_ALLOC_POOL(pMemPool, sizeof(struct D2MapAIPathPosition) * pMapAI->nPathNodes);
	memcpy(pNewMapAI->pPosition, pMapAI->pPosition, sizeof(struct D2MapAIPathPosition) * pMapAI->nPathNodes);
	return pNewMapAI;
}

//D2Common.0x6FD864F0 (#10021)
struct D2MapAI* DRLGPRESET_ChangeMapAI(struct D2MapAI** ppMapAI1, struct D2MapAI** ppMapAI2)
{
	*ppMapAI2 = *ppMapAI1;
	*ppMapAI1 = NULL;
...
void DRLGPRESET_FreeMapAI(void* pMemPool, struct D2MapAI* pMapAI)
{
...
void DRLGPRESET_AddPresetUnitToDrlgMap(void* pMemPool, struct D2DrlgMap* pDrlgMap, struct D2Seed* pSeed)
{
	...
	for (struct D2PresetUnit* pPresetUnit = pDrlgMap->pFile->pPresetUnit; pPresetUnit; pPresetUnit = pPresetUnit->pNext)
	{
...
// Helper function
void DRLGPRESET_SpawnRiver(struct D2DrlgMap* pMazeMap, const struct D2DrlgCoord* tDrlgCoord,void* pDrlgMemPool, struct D2DrlgGrid* pDrlgGrid, int nOffsetX)
{
...
void DRLGPRESET_AddPresetRoomMapTiles(struct D2DrlgRoom* pDrlgRoom)
{
	...
	struct D2DrlgPresetRoom* pMaze = pDrlgRoom->pMaze;
	struct D2DrlgMap* pMazeMap = pDrlgRoom->pMaze->pMap;
	...
	struct D2DrlgGrid tTownOrGraveyardTileInfoGrid = {};
	int aCellOffsets[256] = {};
	int aCellPosBuffer[1024] = {};
	struct D2DrlgGrid tTownOrGraveyardTileTypeGrid = {};
	...
	if (pDrlgRoom->pLevel->nLevelId == LEVEL_BURIALGROUNDS && !pMaze->pTombStoneTiles)
	{
		...
		pMaze->pTombStoneTiles = (struct D2Coord*)D2_ALLOC_POOL(pDrlgRoom->pLevel->pDrlg->pMempool, 6 * sizeof(struct D2Coord));
...
struct D2DrlgRoom* DRLGPRESET_BuildArea(struct D2DrlgLevel* pLevel, struct D2DrlgMap* pDrlgMap, int nFlags, BOOL bSingleRoom)
{
	...
	struct D2DrlgRoom* pDrlgRoom = nullptr;
	if (bSingleRoom)
	{
	...
	else
	{
		...
		struct D2DrlgCoord tDrlgCoord = { 0, 0, 0, 0 };
		int nGridY = 0;
		...
		for (int nY = pDrlgMap->pDrlgCoord.nPosY; nY < nYEnd; nY += 8)
		{
			...
			for (int nX = pDrlgMap->pDrlgCoord.nPosX; nX < nXEnd; nX += 8)
			{
				...
				if (tDrlgCoord.nWidth && tDrlgCoord.nHeight)
					pDrlgRoom = DRLGPRESET_InitPresetRoomData(pLevel, pDrlgMap, &tDrlgCoord, pDrlgMap->pLvlPrestTxtRecord->dwDt1Mask, nGridFlags, DRLGPRESETROOMFLAG_NONE, (struct D2DrlgGrid*)(pDrlgMap->bHasInfo ? nGridFlags : 0));
...
void DRLGPRESET_BuildPresetArea(struct D2DrlgLevel* pLevel, struct D2DrlgGrid* pDrlgGrid, int nFlags, struct D2DrlgMap* pDrlgMap, BOOL bSingleRoom)
{
	...
	if (pDrlgMap->pLvlPrestTxtRecord->dwScan || nPresetTxtRecordNumberPops)
	{
		...
		if (nPresetTxtRecordNumberPops)
		{
			...
			pDrlgMap->pPopsLocation = (struct D2DrlgCoord*)D2_ALLOC_POOL(pLevel->pDrlg->pMempool, sizeof(struct D2DrlgCoord) * nPresetTxtRecordNumberPops);
		}
		...
		memset(pDrlgMap->pPopsLocation, 0x00, sizeof(struct D2DrlgCoord) * nPresetTxtRecordNumberPops);
		...
		{
			struct D2DrlgGrid pTileTypeGrid = {};
			struct D2DrlgGrid pDrlgGrid2 = {};
			int pCellFlags1[256] = {};
			int pCellFlags2[256] = {};
		...
		if (nPresetTxtRecordNumberPops)
		{
			...
			{
				struct D2DrlgCoord* pPopsLocation = &pDrlgMap->pPopsLocation[i];

				int nX1 = MIN(pPopsLocation->nPosX, pPopsLocation->nWidth);
				int nX2 = MAX(pPopsLocation->nPosX, pPopsLocation->nWidth);
				int nY1 = MIN(pPopsLocation->nPosY, pPopsLocation->nHeight);
				int nY2 = MAX(pPopsLocation->nPosY, pPopsLocation->nHeight);
		...
		{
			for (const struct D2PresetUnit* pPresetUnit = pDrlgMap->pFile->pPresetUnit; pPresetUnit; pPresetUnit = pPresetUnit->pNext)
			{
...
void DRLGPRESET_FreeDrlgMap(void* pMemPool, struct D2DrlgMap* pDrlgMap)
{
	...
	struct D2PresetUnit* pPresetUnit = NULL;
	struct D2DrlgMap* pNextMap = NULL;
	struct D2DrlgMap* pMap = NULL;
	...
	while (pMap)
	{
		...
		DRLGVER_FreeVertices(pMemPool, (struct D2DrlgVertex**)&pMap->bHasInfo);
...
int DRLGPRESET_GetLevelPrestIdFromRoomEx(struct D2DrlgRoom* pDrlgRoom)
{
	return pDrlgRoom->pMaze->nLevelPrest;
}

//D2Common.0x6FD881B0 (#10009)
char* DRLGPRESET_GetPickedLevelPrestFilePathFromRoomEx(struct D2DrlgRoom* pDrlgRoom)
{
...
void DRLGPRESET_UpdatePops(struct D2DrlgRoom* pDrlgRoom, int nX, int nY, BOOL bOtherRoom)
{
	...
	{
		struct D2DrlgRoom* pAdjacentRoom = pDrlgRoom->ppRoomsNear[i];
		if (pAdjacentRoom->nType == DRLGTYPE_PRESET && pAdjacentRoom->pRoom)
		{
			const struct D2DrlgMap* pAdjacentRoomMap = pAdjacentRoom->pMaze->pMap;
	...
	if (bUpdate)
	{
		...
		{
			const struct D2DrlgRoom* pAdjacentRoom = pDrlgRoom->ppRoomsNear[nAdjacentRoomIdx];
...
void DRLGPRESET_TogglePopsVisibility(struct D2DrlgRoom* pDrlgRoom, int nPopSubIndex, struct D2DrlgCoord* pDrlgCoord, int nTick, BOOL nCellFlags)
{
	...
	struct D2DrlgCoord pDrlgCoords = {};
	...
	{
		struct D2DrlgTileData* pDrlgTileData = &pDrlgRoom->pTileGrid->pTiles.pWallTiles[i];
...
void DRLGPRESET_InitLevelData(struct D2DrlgLevel* pLevel)
{
	...
	pLevel->pPreset = D2_ALLOC_STRC_POOL(pLevel->pDrlg->pMempool, struct D2DrlgPresetInfo);
