...
struct D2Drlg* DRLG_AllocDrlg(struct D2DrlgAct* pAct, uint8_t nActNo, HD2ARCHIVE hArchive, uint32_t nInitSeed, int nTownLevelId, uint32_t nFlags, struct D2Game* pGame, uint8_t nDifficulty, AUTOMAPFN pfAutoMap, TOWNAUTOMAPFN pfTownAutoMap)
{
...
void DRLG_FreeDrlg(struct D2Drlg* pDrlg)
{
...
struct D2DrlgLevel* DRLG_AllocLevel(struct D2Drlg* pDrlg, int nLevelId)
{
...
struct D2DrlgLevel* DRLG_GetLevel(struct D2Drlg* pDrlg, int nLevelId)
{
	for (struct D2DrlgLevel* pLevel = pDrlg->pLevel; pLevel; pLevel = pLevel->pNextLevel)
	{
...
void DRLG_InitLevel(struct D2DrlgLevel* pLevel)
{
	SEED_InitLowSeed(&pLevel->pSeed, pLevel->nLevelId + pLevel->pDrlg->dwStartSeed);
...
//D2Common.0x6FD74F70
struct D2DrlgRoom* DRLG_GetRoomExFromCoordinates(int nX, int nY, struct D2Drlg* pDrlg, struct D2DrlgRoom* pDrlgRoomHint, struct D2DrlgLevel* pLevel)
{
...
int DRLG_GetLevelTypeFromLevelId(int nLevelId)
{
...
uint8_t DRLG_GetActNoFromLevelId(int nLevelId)
{
...
int DRLG_GetSaveMonstersFromLevelId(int nLevelId)
{
	return DATATBLS_GetLevelDefRecord(nLevelId)->dwSaveMonsters;
}

//D2Common.0x6FD75350 (#10002)
int DRLG_GetLOSDrawFromLevelId(int nLevelId)
{
