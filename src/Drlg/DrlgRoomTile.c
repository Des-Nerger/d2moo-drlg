...
struct D2TileLibraryEntry* DRLGROOMTILE_GetTileCache(struct D2DrlgRoom* pDrlgRoom, int nType, uint32_t nPackedTileInformation)
{
    ...
    int nEntries = D2CMP_10088_GetTiles(pDrlgRoom->pTiles, nType, nStyle, nSequence, ppTileLibraryEntries, ARRAY_SIZE(ppTileLibraryEntries));
	if (nEntries)
	{
		int nMax = 0;
...
// Helper function
static void DRLGROOMTILE_InitTileDataDefaults(struct D2DrlgRoom* pDrlgRoom, struct D2DrlgTileData* pTileData, int nX, int nY, uint32_t nPackedTileInformation, int nTileType, struct D2TileLibraryEntry* pTileLibraryEntry)
{
...
void DRLGROOMTILE_AddTilePresetUnits(struct D2DrlgRoom* pDrlgRoom, struct D2DrlgTileData* pTileData, uint32_t nPackedTileInformation, int nX, int nY, int nTileType)
{
	...
	static const struct D2UnkDrlgRoomTile2 stru_6FDD0F68[] =
	{
		...
		for (int nTileOffsetX = 0; nTileOffsetX < nTileCountX; ++nTileOffsetX)
		{
			...
			if (nTileInformation.bLOS)
			{
				...
				if (nTileInformation.bIsFloor)
				{
					...
					struct D2DrlgRoom* pLinkedRoom = nullptr;
					struct D2DrlgTileData* pLinkedTileData = DRLGROOMTILE_GetLinkedTileData(pDrlgRoom, TRUE, nTileInformation.nPackedValue, nTilePosX, nTilePosY, &pLinkedRoom);
					if (pLinkedTileData)
					{
				...
				{
					struct D2DrlgRoom* pLinkedRoom = nullptr;
					struct D2DrlgTileData* pLinkedTileData = DRLGROOMTILE_GetLinkedTileData(pDrlgRoom, nTileType == TILETYPE_FLOOR, nTileInformation.nPackedValue, nTilePosX, nTilePosY, &pLinkedRoom);
					if (pLinkedTileData)
					{
			...
			if(!bTileAdded)
			{
				if (nTileInformation.bIsFloor)
				{
					struct D2TileLibraryEntry* pTileCache = DRLGROOMTILE_GetTileCache(pDrlgRoom, TILETYPE_FLOOR, nTileInformation.nPackedValue);
				...
				{
					union D2C_PackedTileInformation nCachedTileInfo = { 0 };
					nCachedTileInfo.nTileStyle = 30;
					nCachedTileInfo.nTileSequence = (uint32_t)(pDrlgRoom->pLevel->nLevelId == LEVEL_ARCANESANCTUARY);
					struct D2TileLibraryEntry* pTileCache = DRLGROOMTILE_GetTileCache(pDrlgRoom, 0, nCachedTileInfo.nPackedValue);


					union D2C_PackedTileInformation nArcaneFloorTileInfo = nTileInformation;
				...
				{
					struct D2TileLibraryEntry* pTileCache = DRLGROOMTILE_GetTileCache(pDrlgRoom, nTileType, nTileInformation.nPackedValue);
					struct D2DrlgTileData* pWallTileData = DRLGROOMTILE_InitWallTileData(pDrlgRoom, nullptr, nTilePosX, nTilePosY, nTileInformation.nPackedValue, pTileCache, nTileType);
				...
				{
					struct D2TileLibraryEntry* pTileCache = DRLGROOMTILE_GetTileCache(pDrlgRoom, TILETYPE_SHADOW, nTileInformation.nPackedValue);
...
// Helper functions
static struct D2RoomTile* DRLGROOMTILE_FindDestinationWarpTile(struct D2DrlgRoom* pDrlgRoom, union D2C_PackedTileInformation nTileInformation)
{
	const int nDestinationLevelId = DRLGWARP_GetWarpDestinationFromArray(pDrlgRoom->pLevel, nTileInformation.nTileStyle);
	struct D2RoomTile* pWarpTile = pDrlgRoom->pRoomTiles;
...
static struct D2LvlWarpTxt* DRLGROOMTILE_UpdateAndGetLvlWarpTxtRecord(struct D2RoomTile* pWarpTile, int nTileType)
{
	struct D2LvlWarpTxt* pLvlWarpTxtRecord = pWarpTile->pLvlWarpTxtRecord;
...
void DRLGROOMTILE_LoadWallWarpTiles(struct D2DrlgRoom* pDrlgRoom, struct D2DrlgTileData* pTileData, uint32_t nPackedTileInformation, int nTileType)
{
	...
	struct D2RoomTile* pWarpTile = DRLGROOMTILE_FindDestinationWarpTile(pDrlgRoom, nTileInformation);
	...
	if (nTileInformation.nTileSequence != 0 && nTileInformation.nTileSequence != 4 || DRLGROOMTILE_AddWarp(pDrlgRoom, pDrlgRoom->nTileXPos + pTileData->nPosX, pTileData->nPosY + pDrlgRoom->nTileYPos, nTileInformation.nPackedValue, nTileType))
	{
		...
		const struct D2LvlWarpTxt* pWarpDef = DRLGROOMTILE_UpdateAndGetLvlWarpTxtRecord(pWarpTile, nTileType);
		if (!pWarpDef)
		{
		...
		if (pWarpDef->dwLitVersion)
		{
			...
			struct D2TileLibraryEntry* pTileCache = DRLGROOMTILE_GetTileCache(pDrlgRoom, nTileType, nTileInformation.nPackedValue);
			struct D2DrlgTileData* pDrlgTileData = DRLGROOMTILE_InitWallTileData(pDrlgRoom, &pWarpTile->unk0x0C, pDrlgRoom->nTileXPos + pTileData->nPosX, pTileData->nPosY + pDrlgRoom->nTileYPos, nTileInformation.nPackedValue, pTileCache, nTileType);
			if (pDrlgTileData)
			{
...
void DRLGROOMTILE_LoadFloorWarpTiles(struct D2DrlgRoom* pDrlgRoom, int nX, int nY, uint32_t nPackedTileInformation, int nTileType)
{
	...
	if (pWarpTile)
	{
		...
		if (pWarpDef->dwLitVersion)
		{
			...
			{
				union D2C_PackedTileInformation nDestinationTileInformation = { 0 };
				...
				struct D2TileLibraryEntry* pTileCache = DRLGROOMTILE_GetTileCache(pDrlgRoom, TILETYPE_FLOOR, nDestinationTileInformation.nPackedValue);
				...
				struct D2DrlgTileData* pDestTileData = DRLGROOMTILE_InitFloorTileData(pDrlgRoom, &pWarpTile->unk0x0C, nDestinationTilePosX, nDestinationTilePosY, nDestinationTileInformation.nPackedValue, pTileCache);
				pDestTileData->dwFlags |= MAPTILE_HIDDEN;
			}

			for (int i = 0; i < pDrlgRoom->pTileGrid->nFloors; ++i)
			{
				struct D2DrlgTileData* pFloorTileData = &pDrlgRoom->pTileGrid->pTiles.pFloorTiles[i];
...
struct D2DrlgTileData* DRLGROOMTILE_GetLinkedTileData(struct D2DrlgRoom* pDrlgRoom, BOOL bFloor, uint32_t nPackedTileInformation, int nX, int nY, struct D2DrlgRoom** ppDrlgRoom)
{
	...
	{
		struct D2DrlgRoom* pNearRoomEx = pDrlgRoom->ppRoomsNear[i];

		if (pNearRoomEx != pDrlgRoom && pNearRoomEx->pTileGrid && DRLGROOM_AreXYInsideCoordinatesOrOnBorder(&pNearRoomEx->pDrlgCoord, nX, nY))
		{
			for (struct D2DrlgTileLink* pTileLink = pNearRoomEx->pTileGrid->pMapLinks; pTileLink; pTileLink = pTileLink->pNext)
			{
				if ((!pTileLink->bFloor && !bFloor) || (pTileLink->bFloor && bFloor))
				{
					for (struct D2DrlgTileData* pTileData = pTileLink->pMapTile; pTileData; pTileData = pTileData->unk0x20)
					{
...
void DRLGROOMTILE_AddLinkedTileData(void* pMemPool, struct D2DrlgRoom* pDrlgRoom, int nTileType, uint32_t nPackedTileInformation, int nX, int nY)
{
	...
	{
		struct D2DrlgTileLink* pCurLink;
		...
		if (pCurLink == nullptr)
		{
			struct D2DrlgTileLink* pTileLink = D2_ALLOC_STRC_POOL(pMemPool, struct D2DrlgTileLink);
		...
		struct D2TileLibraryEntry* pTileCache = DRLGROOMTILE_GetTileCache(pDrlgRoom, nTileType, nPackedTileInformation);
...
void DRLGROOMTILE_LinkedTileDataManager(void* pMemPool, struct D2DrlgRoom* pDrlgRoom1, struct D2DrlgRoom* pDrlgRoom2, struct D2DrlgTileData* pTileData, int nTileType, uint32_t nPackedTileInformation, int nX, int nY)
{
	...
	{
		struct D2TileLibraryEntry* pTileCache = DRLGROOMTILE_GetTileCache(pDrlgRoom2, nTileType, nPackedTileInformation);
		pTileData->nTileType = nTileType;
...
void DRLGROOMTILE_GetCreateLinkedTileData(void* pMemPool, struct D2DrlgRoom* pDrlgRoom, int nTileType, uint32_t nPackedTileInformation, int nX, int nY)
{
	...
	for (int i = 0; i < pDrlgRoom->nRoomsNear; ++i)
	{
		...
		if (pNearRoomEx != pDrlgRoom && pNearRoomEx->pTileGrid && DRLGROOM_AreXYInsideCoordinatesOrOnBorder(&pNearRoomEx->pDrlgCoord, nX, nY))
		{
			struct D2DrlgTileLink* pTileLink = pNearRoomEx->pTileGrid->pMapLinks;
			while (pTileLink)
			{
				struct D2DrlgTileData* pTileData = pTileLink->pMapTile;
...
void DRLGROOMTILE_CountAllTileTypes(struct D2DrlgRoom* pDrlgRoom, struct D2DrlgGrid* pTileInfoGrid, BOOL bCheckCoordinatesValidity, BOOL bKillEdgeX, BOOL bKillEdgeY)
{
	...
	for (int nY = 0; nY < nTileCountY; ++nY)
	{
		for (int nX = 0; nX < nTileCountX; ++nX)
		{
			...
			union D2C_PackedTileInformation nTileInformation = { (uint32_t)*pFlags };
			if (nTileInformation.bIsWall)
			{
...
void DRLGROOMTILE_CountWallWarpTiles(struct D2DrlgRoom* pDrlgRoom, struct D2DrlgGrid* pTileInfoGrid, struct D2DrlgGrid* pTileTypeGrid, BOOL bKillEdgeX, BOOL bKillEdgeY)
{
	...
	for (int nY = 0; nY < nTileCountY; ++nY)
	{
		for (int nX = 0; nX < nTileCountX; ++nX)
		{
			...
			case TILETYPE_WALL_LEFT_EXIT:
			case TILETYPE_WALL_RIGHT_EXIT:
				if ((union D2C_PackedTileInformation){ (uint32_t)DRLGGRID_GetGridEntry(pTileInfoGrid, nX, nY) }.bHidden)
				{
...
void DRLGROOMTILE_AllocTileGrid(struct D2DrlgRoom* pDrlgRoom)
{
	...
	{
		pDrlgRoom->pTileGrid = D2_CALLOC_STRC_POOL(pDrlgRoom->pLevel->pDrlg->pMempool, struct D2DrlgTileGrid);
	}
}
...
void DRLGROOMTILE_AllocTileData(struct D2DrlgRoom* pDrlgRoom)
{
	...
	{
		pRoomTiles->pFloorTiles = (struct D2DrlgTileData*)D2_CALLOC_POOL(pMemPool, sizeof(struct D2DrlgTileData) * pRoomTiles->nFloors);
	}

	if (pRoomTiles->nWalls)
	{
		pRoomTiles->pWallTiles = (struct D2DrlgTileData*)D2_CALLOC_POOL(pMemPool, sizeof(struct D2DrlgTileData) * pRoomTiles->nWalls);
	}
	...
	{
		pRoomTiles->pRoofTiles = (struct D2DrlgTileData*)D2_CALLOC_POOL(pMemPool, sizeof(struct D2DrlgTileData) * pRoomTiles->nRoofs);
	}
}
...
void DRLGROOMTILE_ReallocRoofTileGrid(void* pMemPool, struct D2DrlgTileGrid* pTileGrid, int nAdditionalRoofs)
{
	if (nAdditionalRoofs)
	{
		pTileGrid->pTiles.pRoofTiles = (struct D2DrlgTileData*)D2_REALLOC_POOL(pMemPool, pTileGrid->pTiles.pRoofTiles, sizeof(struct D2DrlgTileData) * (nAdditionalRoofs + pTileGrid->pTiles.nRoofs));
...
int DRLGROOMTILE_GetNumberOfShadowsFromRoom(struct D2ActiveRoom* pRoom)
{
	if (pRoom)
	{
...
void DRLGROOMTILE_FreeTileGrid(struct D2DrlgRoom* pDrlgRoom)
{
	...
	if (pTileGrid)
	{
		...
		struct D2DrlgTileLink* pTileLink = pTileGrid->pMapLinks;
		while (pTileLink)
		{
			struct D2DrlgTileLink* pNextTileLink = pTileLink->pNext;
			D2_FREE_POOL(pDrlgRoom->pLevel->pDrlg->pMempool, pTileLink);
			pTileLink = pNextTileLink;
		}
		pTileGrid->pMapLinks = nullptr;

		struct D2DrlgAnimTileGrid* pAnimTileGrid = pTileGrid->pAnimTiles;
		while (pAnimTileGrid)
		{
			struct D2DrlgAnimTileGrid* pNextAnimTileGrid = pAnimTileGrid->pNext;
