...
// Helper functions
static bool DRLGMAZE_CheckIfRoomOverlapsOrth(struct D2DrlgRoom* pNewRoomEx, struct D2DrlgOrth* pDrlgOrth)
{
	while (pDrlgOrth)
	{
...
static bool DRLGMAZE_CheckIfRoomOverlapsAythingOtherThanParent(struct D2DrlgRoom* pNewRoomEx, struct D2DrlgRoom* pParentRoomEx)
{
...
static bool DRLGMAZE_OrthWithDirectionExists(struct D2DrlgRoom* pDrlgRoom, int nDirection)
{
	struct D2DrlgOrth* pDrlgOrth = pDrlgRoom->pDrlgOrth;
...
void __fastcall DRLGMAZE_PickRoomPreset(struct D2DrlgRoom* pDrlgRoom, BOOL bResetFlag)
{
	...
	for (struct D2DrlgOrth* pDrlgOrth = pDrlgRoom->pDrlgOrth; pDrlgOrth; pDrlgOrth = pDrlgOrth->pNext)
	{
...
void __fastcall DRLGMAZE_GenerateLevel(struct D2DrlgLevel* pLevel)
{
	...
	static const struct D2MazeLevelId nAct1CaveDOEIds[] =
	{
	...
	static const struct D2MazeLevelId nAct1CaveDownIds[] =
	{
	...
	static const struct D2MazeLevelId nAct1CaveColdcrowIds[] =
	{
	...
	static const struct D2MazeLevelId nAct1CaveNextIds[] =
	{
	...
	static const struct D2MazeLevelId nAct1CryptPrevIds[] =
	{
	...
	static const struct D2MazeLevelId nAct1CryptSpecialIds[] =
	{
	...
	static const struct D2MazeLevelId nAct1CryptChestIds[] =
	{
	...
	static const struct D2MazeLevelId nAct1CryptNextIds[] =
	{
	...
	static const struct D2MazeLevelId nAct1JailPrevIds[] =
	{
	...
	static const struct D2MazeLevelId nAct1JailCathIds[] =
	{
	...
	static const struct D2MazeLevelId nAct1JailNextIds[] =
	{
	...
	static const struct D2MazeLevelId nAct1JailWaypointIds[] =
	{
	...
	static const struct D2MazeLevelId nAct1JailPitspawnIds[] =
	{
	...
	static const struct D2MazeLevelId nAct1CatacombsNextIds[] =
	{
	...
	static const struct D2MazeLevelId nAct1CatacombsWaypointIds[] =
	{
	...
	static const struct D2MazeLevelId nAct2ArcaneSummonerIds[] =
	{
...
struct D2DrlgRoom* __fastcall DRLGMAZE_AddAdjacentMazeRoom(struct D2DrlgRoom* pDrlgRoom, int nDirection, BOOL bMergeRooms)
{
	...
	for (struct D2DrlgOrth* i = pDrlgRoom->pDrlgOrth; i; i = i->pNext)
	{
	...
	{
		for (struct D2DrlgRoom* i = pNewRoomEx->pLevel->pFirstRoomEx; i; i = i->pDrlgRoomNext)
		{
...
BOOL __fastcall DRLGMAZE_LinkMazeRooms(struct D2DrlgRoom* pDrlgRoom1, struct D2DrlgRoom* pDrlgRoom2, int nDirection)
{
	...
	for (struct D2DrlgOrth* pDrlgOrth = pDrlgRoom2->pDrlgOrth; pDrlgOrth; pDrlgOrth = pDrlgOrth->pNext)
	{
...
void __fastcall DRLGMAZE_MergeMazeRooms(struct D2DrlgRoom* pDrlgRoom)
{
	...
	{
		for (struct D2DrlgRoom* i = pDrlgRoom->pLevel->pFirstRoomEx; i; i = i->pDrlgRoomNext)
		{
...
static void PlaceLavaPreset(struct D2DrlgRoom* pFirstRoomEx, int nSet)
{

	int nDirection = dword_6FDCE850[nSet][1];

	struct D2DrlgRoom* pNewRoomEx = DRLGROOM_AllocRoomEx(pFirstRoomEx->pLevel, DRLGTYPE_PRESET);
	pNewRoomEx->nTileWidth = pNewRoomEx->pLevel->pMaze->dwSizeX;
	pNewRoomEx->nTileHeight = pNewRoomEx->pLevel->pMaze->dwSizeY;
...
{		
	...
	struct D2DrlgRoom* pLevelFirstRoom = pLevel->pFirstRoomEx;
...
void __fastcall DRLGMAZE_FillBlankMazeSpaces(struct D2DrlgLevel* pLevel, int nLevelPrest, struct D2DrlgRoom* pIgnoreRoomEx)
{
	...
	if (nRooms > 0)
	{
		struct D2DrlgRoom** pDrlgRoomArray = (struct D2DrlgRoom**)D2_ALLOC_POOL(pLevel->pDrlg->pMempool, sizeof(struct D2DrlgRoom*) * nRooms);

		// Copy rooms pointers
		{
			struct D2DrlgRoom* pTemp = pLevel->pFirstRoomEx;
		...
		for (int i = 0; i < nRooms; ++i)
		{
			struct D2DrlgRoom* pCurrentRoomEx = pDrlgRoomArray[i];
			if (pCurrentRoomEx != pIgnoreRoomEx)
			{
				for (int j = 0; j < 8; ++j)
				{
					struct D2DrlgRoom* pNewRoomEx = DRLGROOM_AllocRoomEx(pCurrentRoomEx->pLevel, DRLGTYPE_PRESET);
					pNewRoomEx->nTileWidth = pNewRoomEx->pLevel->pMaze->dwSizeX;
					pNewRoomEx->nTileHeight = pNewRoomEx->pLevel->pMaze->dwSizeY;
...
void __fastcall DRLGMAZE_PlaceAct2TombPrev_Act5BaalPrev(struct D2DrlgLevel* pLevel)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		struct D2DrlgRoom* pDrlgRoom = DRLGROOM_AllocRoomEx(pLevelFirstRoomEx->pLevel, DRLGTYPE_PRESET);
...
//Helper function
static struct D2DrlgRoom* DRLGMAZE_PlaceRoomForArcaneBranch(struct D2DrlgRoom* pParentRoom, int nArcaneMapBranch)
{
	struct D2DrlgRoom* pDrlgRoom = DRLGMAZE_PlaceAdjacentPresetRoom(pParentRoom, nArcaneMapBranch % 4, TRUE);
	if (pDrlgRoom)
	{
...
void __fastcall DRLGMAZE_PlaceArcaneSanctuary(struct D2DrlgLevel* pLevel)
{
	...
	struct D2DrlgRoom* pDrlgRoomArray[nRoomsPerBranch * nBranchDirections] = {};

	for (int nBranchDirection = 0; nBranchDirection < nBranchDirections; ++nBranchDirection)
	{
		struct D2DrlgRoom* pParentRoom = pLevelFirstRoomEx;
		for (int nBranchRoomIdx = 0; nBranchRoomIdx < nRoomsPerBranch; nBranchRoomIdx++)
		{
			...
			struct D2DrlgRoom* pNewRoom = DRLGMAZE_PlaceRoomForArcaneBranch(pParentRoom, nRoomDirection);
...
void __fastcall DRLGMAZE_ScanReplaceSpecialAct2SewersPresets(struct D2DrlgLevel* pLevel)
{
	...
	static const struct D2MazeLevelId nAct2SewerNextIds[] =
	{
	...
	static const struct D2MazeLevelId nAct2SewerWaypointIds[] =
	{
	...
	static const struct D2MazeLevelId nAct2SewerRadamentIds[] =
	{
	...
	static const struct D2MazeLevelId nAct2SewerChestIds[] =
	{
	...
	const struct D2MazeLevelId* pMazeLevelIds = NULL;
	struct D2DrlgRoom* pDrlgRoom1 = NULL;
	struct D2DrlgRoom* pDrlgRoom2 = NULL;
	struct D2DrlgRoom* pDrlgRoom3 = NULL;
	struct D2DrlgRoom* pDrlgRoom4 = NULL;
	struct D2DrlgRoom* pDrlgRoom5 = NULL;
	int nDirection = 0;
	int nRand = 0;
...
BOOL __fastcall DRLGMAZE_CheckIfMayPlaceAdjacentPresetRoom(struct D2DrlgRoom* pDrlgRoom, int nDirection)
{
	...
	struct D2DrlgRoom* pNewRoomEx = DRLGMAZE_PlaceAdjacentPresetRoom(pDrlgRoom, nDirection, FALSE);
...
void __fastcall DRLGMAZE_PlaceAct2TombStuff(struct D2DrlgLevel* pLevel)
{
	...
	static const struct D2MazeLevelId nAct2TombWaypointIds[] =
	{
	...
	static const struct D2MazeLevelId nAct2TombChestIds[] =
	{
	...
	static const struct D2MazeLevelId nAct2TombLeatherarmIds[] =
	{
	...
	static const struct D2MazeLevelId nAct2TombCubeIds[] =
	{
	...
	static const struct D2MazeLevelId nAct2TombTreasureIds[] =
	{
	...
	static const struct D2MazeLevelId nAct2TombTalRashaIds[] =
	{
	...
	static const struct D2MazeLevelId nAct2TombKaaIds[] =
	{
	...
	const struct D2MazeLevelId* pMazeLevelIds = NULL;
	struct D2DrlgPresetRoom* pPresetRoom = NULL;
	int nDirection = 0;
...
void __fastcall DRLGMAZE_PlaceAct2LairStuff(struct D2DrlgLevel* pLevel)
{
	...
	static const struct D2MazeLevelId nAct2LairSpecialIds[] =
	{
...
void __fastcall DRLGMAZE_PlaceAct3DungeonStuff(struct D2DrlgLevel* pLevel)
{
	...
	static const struct D2MazeLevelId nAct3DungeonNextIds[] =
	{
...
void __fastcall DRLGMAZE_PlaceAct3SewerStuff(struct D2DrlgLevel* pLevel)
{
	...
	static const struct D2MazeLevelId nAct3SewerChestIds[] =
	{
...
void __fastcall DRLGMAZE_PlaceAct3MephistoStuff(struct D2DrlgLevel* pLevel)
{
	...
	static const struct D2MazeLevelId nAct3MephistoWaypointIds[] =
	{
	...
	static const struct D2MazeLevelId nAct3MephistoNextIds[] =
	{
...
void __fastcall DRLGMAZE_PlaceAct5BaalStuff(struct D2DrlgLevel* pLevel)
{
	...
	static const struct D2MazeLevelId nAct5BaalWaypointIds[] =
	{
...
void __fastcall DRLGMAZE_PlaceAct1Barracks(struct D2DrlgLevel* pLevel)
{
	...
	static const struct D2MazeLevelId nAct1BarracksNextIds[] =
	{
	...
	static const struct D2MazeLevelId nAct1BarracksForgeIds[] =
	{
	...
	const struct D2MazeLevelId* pMazeLevelIds = NULL;
	struct D2DrlgLevel* pOuterCloisterLevel = NULL;
	struct D2DrlgRoom* pBarracksRoomEx = NULL;
	struct D2DrlgRoom* pDrlgRoom = NULL;
	int nTileMinX = 0;
	int nTileMinY = 0;
	int nTileMaxX = 0;
...
void __fastcall DRLGMAZE_PlaceAct4Lava(struct D2DrlgLevel* pLevel)
{
	...
	{
		struct D2DrlgLevel* pChaosSanctum = DRLG_GetLevel(pLevel->pDrlg, LEVEL_CHAOSSANCTUM);

		struct D2DrlgRoom* pParentRoom = DRLGMAZE_GetFreeLocationForRoomNorth(pLevel);

		struct D2DrlgRoom* pNewRoomEx = DRLGROOM_AllocRoomEx(pParentRoom->pLevel, DRLGTYPE_PRESET);
		DRLGMAZE_SetRoomSize(pNewRoomEx);
		...
		struct D2DrlgRoom* pBridgeRoomEx1 = DRLGMAZE_PlaceAdjacentPresetRoom(pParentRoom, ALTDIR_NORTH, FALSE);
		if (pBridgeRoomEx1)
		{
			DRLGMAZE_PickRoomPreset(pParentRoom, 1);
		...
		struct D2DrlgRoom* pBridgeRoomEx2 = DRLGMAZE_PlaceAdjacentPresetRoom(pBridgeRoomEx1, ALTDIR_NORTH, FALSE);
		if (pBridgeRoomEx2)
		{
			DRLGMAZE_SetPickedFileAndPresetId(pBridgeRoomEx2, LVLPREST_ACT4_BRIDGE_2, -1, FALSE);
		}

		struct D2DrlgRoom* pBridgeRoomEx3 = DRLGMAZE_PlaceAdjacentPresetRoom(pBridgeRoomEx2, ALTDIR_NORTH, FALSE);
		if (pBridgeRoomEx3)
		{
...
void __fastcall DRLGMAZE_PlaceAct5IceStuff(struct D2DrlgLevel* pLevel)
{
	...
	static const struct D2MazeLevelId nAct5IceNextIds[] =
	{
	...
	static const struct D2MazeLevelId nAct5IceDownIds[] =
	{
	...
	static const struct D2MazeLevelId nAct5IceThemeIds[] =
	{
	...
	static const struct D2MazeLevelId nAct5IceWaypointIds[] =
	{
