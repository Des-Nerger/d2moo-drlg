...
void __fastcall DRLGLOGIC_FreeDrlgCoordList(struct D2DrlgRoom* pDrlgRoom)
{
	...
	if (pDrlgRoom->pLogicalRoomInfo)
	{
		...
		struct D2RoomCoordList* pRoomCoordList = pRoomCoordList = pDrlgRoom->pLogicalRoomInfo->pCoordList;
		while (pRoomCoordList)
		{
			struct D2RoomCoordList* pNext = pRoomCoordList->pNext;
...
void __fastcall DRLGLOGIC_InitializeDrlgCoordList(struct D2DrlgRoom* pDrlgRoom, struct D2DrlgGrid* pTileTypeGrid, struct D2DrlgGrid* pFloorGrid, struct D2DrlgGrid* pWallGrid)
{
	...
	struct D2DrlgLogicalRoomInfo* pDrlgCoordList = D2_CALLOC_STRC_POOL(pDrlgRoom->pLevel->pDrlg->pMempool, struct D2DrlgLogicalRoomInfo);
	pDrlgRoom->pLogicalRoomInfo = pDrlgCoordList;

	struct D2DrlgRoomTiles* pDrlgRoomTiles = &pDrlgRoom->pTileGrid->pTiles;
	for (int i = 0; i < pDrlgRoomTiles->nWalls; ++i)
	{
		struct D2DrlgTileData* pWallTileData = &pDrlgRoomTiles->pWallTiles[i];
	...
	{
		struct D2DrlgRoom* pDrlgRoomNear = pDrlgRoom->ppRoomsNear[i];
		if (pDrlgRoomNear != pDrlgRoom && pDrlgRoomNear->pTileGrid)
		{
			for (struct D2DrlgTileLink* pDrlgTileLink = pDrlgRoomNear->pTileGrid->pMapLinks; pDrlgTileLink; pDrlgTileLink = pDrlgTileLink->pNext)
			{
				if (!pDrlgTileLink->bFloor)
				{
					for (struct D2DrlgTileData* pDrlgTileData = pDrlgTileLink->pMapTile; pDrlgTileData; pDrlgTileData = pDrlgTileData->unk0x20)
					{
	...
	struct D2UnkDrlgLogic tDRLGLogicUnkStrc = {};
	...
	for (int j = 0; j <= pDrlgRoom->nTileHeight; ++j)
	{
		for (int i = 0; i <= pDrlgRoom->nTileWidth; ++i)
		{
			if (!(DRLGGRID_GetGridEntry(tDRLGLogicUnkStrc.field_4, i, j) & 0x10000000))
			{
				...
				union D2C_PackedTileInformation nFloorPackedInfo = { (uint32_t)DRLGGRID_GetGridEntry(tDRLGLogicUnkStrc.pFloorGrid, i, j) };
	...
	pDrlgCoordList->pCoordList = (struct D2RoomCoordList *)D2_CALLOC_POOL(pDrlgRoom->pLevel->pDrlg->pMempool, sizeof(struct D2RoomCoordList) * pDrlgCoordList->nLists);
...
{
	static const struct D2Coord stru_6FDCE5C8[] =
	{
		{ 1, 0 },
		{ 0, 1 },
		{ -1, 0 },
...
void __fastcall sub_6FD76A90(struct D2DrlgRoom* pDrlgRoom1, struct D2DrlgRoom* pDrlgRoom2, int nX, int nY)
{
	...
	{
		struct D2RoomCoordList* pRoomCoordList1 = sub_6FD77110(pDrlgRoom1, nX * 5, nY * 5);
		struct D2RoomCoordList* pRoomCoordList2 = sub_6FD77110(pDrlgRoom2, nX * 5, nY * 5);
...
void __fastcall sub_6FD76B90(struct D2DrlgRoom* pDrlgRoom, int nIndex1, int nIndex2, BOOL bNode)
{
	...
	struct D2RoomCoordList* pRoomCoordList = pDrlgRoom->pLogicalRoomInfo->pCoordList;
	if (pRoomCoordList)
	{
...
void __fastcall DRLGLOGIC_AssignCoordListsForGrids(struct D2DrlgRoom* pDrlgRoom, struct D2DrlgLogicalRoomInfo* pDrlgCoordList, int nLists)
{
	...
	for (int nY = 0; nY < nHeight; ++nY)
	{
		for (int nX = 0; nX < nWidth; ++nX)
		{
			...
			{
				struct D2RoomCoordList* pRoomCoordList = D2_CALLOC_STRC_POOL(pDrlgRoom->pLevel->pDrlg->pMempool, struct D2RoomCoordList);
...
{
	struct D2DrlgLogicalRoomInfo* pLogicalRoomInfo = D2_CALLOC_STRC_POOL(pDrlgRoom->pLevel->pDrlg->pMempool, struct D2DrlgLogicalRoomInfo);
	...
	pLogicalRoomInfo->pCoordList = D2_CALLOC_STRC_POOL(pDrlgRoom->pLevel->pDrlg->pMempool, struct D2RoomCoordList);
...
{
	struct D2RoomCoordList* pRoomCoordList = sub_6FD77110(pDrlgRoom, nX, nY);
...
struct D2RoomCoordList* __fastcall sub_6FD77110(struct D2DrlgRoom* pDrlgRoom, int nX, int nY)
{
	...
	{
		return (struct D2RoomCoordList*)DRLGGRID_GetGridEntry(&pDrlgRoom->pLogicalRoomInfo->pIndexY, nX / 5 - pDrlgRoom->nTileXPos, nY / 5 - pDrlgRoom->nTileYPos);
	}
}
