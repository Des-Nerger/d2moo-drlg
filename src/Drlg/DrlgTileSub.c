...
void __fastcall DRLGTILESUB_AddSecondaryBorder(struct D2UnkOutdoor* a1)
{
	...
	while (pLvlSubTxtRecord->dwType == a1->nLvlSubId)
	{
		...
		if (pLvlSubTxtRecord->pDrlgFile->nSubstGroups > 0)
		{
			...
			{
				pSubstGroup = &((struct D2DrlgSubstGroup*)(pLvlSubTxtRecord->pDrlgFile->pSubstGroups))[(nRand + j) % pLvlSubTxtRecord->pDrlgFile->nSubstGroups];
		...
		while (nThemeFlag)
		{
			if (nThemeFlag & 1)
			{
				...
				if (pLvlSubTxtRecord->dwCheckAll)
				{
					...
					{
						pSubstGroup = &((struct D2DrlgSubstGroup*)pLvlSubTxtRecord->pDrlgFile->pSubstGroups)[nCurSubstGroupIndex];
...
void __fastcall sub_6FD8ACE0(void* pMemPool, int nX, int nY, struct D2UnkOutdoor2* a4, struct D2DrlgSubstGroup* pSubstGroup, struct D2LvlSubTxt* pLvlSubTxtRecord, int a7)
{
	...
	for (struct D2PresetUnit* pPresetUnit = pLvlSubTxtRecord->pDrlgFile->pPresetUnit; pPresetUnit; pPresetUnit = pPresetUnit->pNext)
	{
...
void __fastcall DRLGTILESUB_DoSubstitutions(struct D2UnkOutdoor2* pOutdoorLevel, struct D2LvlSubTxt* pLvlSubTxtRecord)
{
	...
	for (int xyz = 0; xyz < pLvlSubTxtRecord->nMax[pOutdoorLevel->nSubTheme]; ++xyz)
	{
		...
		struct D2DrlgSubstGroup* pSubstGroup = &pLvlSubTxtRecord->pDrlgFile->pSubstGroups[substGroupIdx];
